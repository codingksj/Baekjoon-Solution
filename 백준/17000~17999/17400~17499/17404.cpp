...
// ---------- 사용자 정의 함수 및 연산자 ----------
int FindMinCost(const Matrix2D<int>& rgbs, const int N);
...
int main() {
    ...
    rgbs.m2 = Load2DVector<int>(N, 3);
    minCost = FindMinCost(rgbs, N);
...
int FindMinCost(const Matrix2D<int>& rgbs, const int N) {
    Matrix2D<int> dp = Matrix2D<int>(N, 3, INF);
    int minCost = INF;
    int cost1, cost2;

    for (int start = 0; start < 3; start++) {
        for (int cur = 0; cur < 3; cur++) {
            dp.m2[0][cur] = (start == cur) ? rgbs.m2[0][cur] : INF;
        }
        for (int house = 1; house < N; house++) {
            for (int i = 0; i < 3; i++) {
                cost1 = dp.m2[house - 1][(i + 1) % 3];
                cost2 = dp.m2[house - 1][(i + 2) % 3];
                dp.m2[house][i] = rgbs.m2[house][i] + min(cost1, cost2);
            }
        }
        for (int end = 0; end < 3; end++) {
            if (start != end) {
                minCost = min(minCost, dp.m2[N - 1][end]);
            }
        }
    }
