...
void Solve() {
    wordCnt = count(s.begin(), s.end(), ' ') + 1;
    sz = s.length();
    for (int i = 0; i < sz - 1; i++) {
        if (s.substr(i, 2) == "ae") {
            aeCnt++;
            size_t pos = s.find(' ', i + 1);
            i = (pos == string::npos) ? sz : pos;
        }
    }
    isSweden = (5 * aeCnt >= 2 * wordCnt) && (aeCnt > 0);
};

void Output() {
    cout << (isSweden ? "dae ae ju traeligt va" : "haer talar vi rikssvenska") << "\n";
    return;
};
