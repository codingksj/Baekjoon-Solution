...
bool IsCross(pair<PLL, PLL> ln1, pair<PLL, PLL> ln2);
void Output();
...
bool Input() {
    points = LoadVector<LL, LL>(4);

    return true;
};

void Solve() {
    ln1 = make_pair(points[0], points[1]);
    ln2 = make_pair(points[2], points[3]);
    isCross = IsCross(ln1, ln2);
}

int CCW(const PLL& a, const PLL& b, const PLL& c) {
    LL s = a.first * (b.second - c.second)
