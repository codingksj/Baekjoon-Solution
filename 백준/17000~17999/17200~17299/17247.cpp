...
// ---------- 사용자 정의 함수 및 연산자 ----------
pair<Pii, Pii> FindStartAndEnd(const Matrix2D<int>& city, const int R, const int C);
...
int main() {
    ...
    Matrix2D<int> city;
    pair<Pii, Pii> startAndEnd;
    int R, C;

    cin >> R >> C;

    city.m2 = Load2DVector<int>(R, C);
    startAndEnd = FindStartAndEnd(city, R, C);

    cout << startAndEnd.first - startAndEnd.second << "\n";
