...
// ---------- 사용자 정의 함수 및 연산자 ----------
vector<int> FindPossibleRows(const Words& board, const int R, const int C);
vector<int> FindPossibleCols(const Words& board, const int R, const int C);
int FindMinMove(const vector<int>& rows, const vector<int>& cols, const Pii& me);
...
int main() {
    ...
    board = LoadVector<string>(R); 
    rows = FindPossibleRows(board, R, C);
    cols = FindPossibleCols(board, R, C);
    minMove = FindMinMove(rows, cols, me);
...
int FindMinMove(const vector<int>& rows, const vector<int>& cols, const Pii& me) {
    int minMove = MILLION;
    int curMove;
    int startR = me.first - 1;
    int startC = me.second - 1;

    for (const int& row : rows) {
        for (const int& col : cols) {
            curMove = abs(row - startR) + abs(col - startC);
            minMove = min(minMove, curMove);
        }
    }
