...
template<typename T1, typename T2> void PrintPair(const pair<T1, T2>& p, string_view sep = " ", string_view suffix = "\n") {
	print("{}{}{}{}", p.first, sep, p.second, suffix);
}
template<typename T> void PrintVec(const vector<T>& V, string_view sep = " ", string_view suffix = "\n") {
	print("{}", format("{}", views::all(V) | views::join_with(sep), suffix));
}
template<typename T1, typename T2> void PrintVec(const vector<pair<T1, T2>>& V, string_view sep = " ", string_view suffix = "\n") {
	for (const auto& p : V) {	PrintPair(p, sep, suffix);	}
}
template<typename T> void Print2DVec(const vector<vector<T>>& V, string_view sep = " ", string_view suffix = "\n") {
	for (const auto& row : V) {	PrintVec(row, sep, suffix);	}
}
template<typename T1, typename T2> void Print2DVec(const vector<vector<pair<T1, T2>>>& V, string_view sep = " ", string_view suffix = "\n") {
	for (const auto& row : V) {	PrintVec<T1, T2>(row, sep, suffix);	}
}
...
enum GRAPH_CONSTS : int { MAX_VERTEX = 500, MAX_EDGES = 40000, MAX_DEPTH = 10 };
...
bool Input();
void Solve();
int DFS(int depth, int sz, int stR, int stC);
void Output();
...
int DFS(int depth, int sz, int r, int c) {
	if (depth == K) {
		return A[r][c];
	}
	int half = sz >> 1;
	int v0 = DFS(depth + 1, half, r, c);
	int v1 = DFS(depth + 1, half, r, c + half);
	int v2 = DFS(depth + 1, half, r + half, c);
	int v3 = DFS(depth + 1, half, r + half, c + half);
	int mx1 = max(v0, v1), mn1 = min(v0, v1);
	int mx2 = max(v2, v3), mn2 = min(v2, v3);

	return max(min(mx1, mx2), max(mn1, mn2));
};
