...
void Input();
void Solve();
void DFS(const int r, const int c, const int dir);
void Output();
...
void DFS(const int r, const int c, const int dir) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        int nr = r + dr[i];
        int nc = c + dc[i];
        if (nr < 0 || nr >= N || nc < 0 || nc >= N || dfsChk[nr][nc] || house[nr][nc]) {
            continue;
        }
        if ((i < 2) || !(house[nr - 1][nc] + house[nr][nc - 1])) {
            dfsChk[nr][nc] = true;
            DFS(nr, nc, i);
            dfsChk[nr][nc] = false;
        }
