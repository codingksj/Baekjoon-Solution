...
// ---------- 사용자 정의 함수 및 연산자 ----------
void DFS(const int depth, const int res);
int Operation(const int op, const int a, const int b);
...
// ---------- 전역 변수 ----------
vector<int> seq;
vector<int> operCounts;
int minRes = BILLION + 1;
int maxRes = -(BILLION + 1);
int N;
...
void DFS(const int depth, const int res) {
    ...
    for (int i = 0; i < 4; i++) {
        if (operCounts[i] > 0) {
            operCounts[i]--;
            DFS(depth + 1, Operation(i, res, seq[depth + 1]));
            operCounts[i]++;
        }
    }
};
