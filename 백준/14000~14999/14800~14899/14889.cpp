...
// ---------- 사용자 정의 함수 및 연산자 ----------
void DFS(const int depth, const int prevIndex);
int CalcDiff();
// ---------- 열거형 상수 ----------
...
int CalcDiff() {
    ...
    for (int i = 0; i < N; i++) {
        for (int j = i + 1; j < N; j++) {
            (visCheck[i] && visCheck[j]) && (teamStart += (stats.m2[i][j] + stats.m2[j][i]));
            (!visCheck[i] && !visCheck[j]) && (teamLink += (stats.m2[i][j] + stats.m2[j][i]));
        }
    }
