...
void Solve() {
    while (canClean) {
        ...
        for (int i = 0; i < 4; i++) {
            int ndir = (dir + 3 - i) % 4;
            const auto& [dr, dc] = DT4[ndir];
            int nr = r + dr;
            int nc = c + dc;
            if (nr < 0 || nr >= N || nc < 0 || nc >= M || room[nr][nc] != EMPTY) {
                continue;
            }
            dir = ndir;
            r = nr;
        ...
        if (!canClean) {
            const auto& [dr, dc] = DT4[dir];
            int nr = r - dr;
            int nc = c - dc;
            if (nr < 0 || nr >= N || nc < 0 || nc >= M || room[nr][nc] == WALL) {
                break;
            }
            else {
                canClean = true;
