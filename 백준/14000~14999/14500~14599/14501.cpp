...
// ---------- 사용자 정의 함수 ----------
int CalcMaxProfit(const vector<Pii>& V, const int N);
...
int main() {
    ...
    V = LoadVector<int, int>(N);
    maxProfit = CalcMaxProfit(V, N);
...
int CalcMaxProfit(const vector<Pii>& V, const int N) {
    vector<int> dp = InitVector<int>(N + 1, 0);
    int spendDay;
    int profit;
    ...
    for (int i = 0; i < N; i++) {
        spendDay = V[i].first;
        profit = V[i].second;
        for (int day = i + spendDay; day <= N; day++) {
            dp[day] = max(dp[day], dp[i] + profit);
        }
    }
