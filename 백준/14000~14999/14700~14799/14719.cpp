...
//사용자 지정 함수
int CalcTotalRainDrop(const vector<int>& blocks, const int H, const int W);
...
int main() {
    ...
    blocks = LoadVector<int>(W);
    rainDrop = CalcTotalRainDrop(blocks, H, W);
...
int CalcTotalRainDrop(const vector<int>& blocks, const int H, const int W) {
    ...
    for (int w = 1; w < W - 1; w++) {
        ...
        for (int i = w - 1; i >= 0; i--) {
            leftH = max(leftH, blocks[i]);
        }
        for (int i = w + 1; i <= W - 1; i++) {
            rightH = max(rightH, blocks[i]);
        }

        rainDrop = max(0, min(leftH, rightH) - blocks[w]);
        totalRainDrop += rainDrop;
    }

    return totalRainDrop;
};
