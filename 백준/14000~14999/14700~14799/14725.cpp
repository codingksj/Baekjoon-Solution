...
class Trie {
	...
	public:
		...
		void Insert(const Words& words) {
			Trie* curr = this;
			for (const auto& w : words) {
				if (!curr->children[w]) {
					curr->children[w] = new Trie();
					curr->children[w]->depth = curr->depth + 1;
				}
		...
		void PrintAll(Trie* curr) {
			for (const auto& [word, child] : curr->children) {
				cout << string(child->depth * 2, '-') << word << '\n';
				PrintAll(child);
			}
		}
};
