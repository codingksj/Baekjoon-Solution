...
// ---------- 사용자 정의 함수 및 연산자 ----------
void InsertEdge(Matrix2D<LL>& graph, const int startV, const int endV, const int w);
Matrix2D<LL> Multiplication(const Matrix2D<LL> A, const Matrix2D<LL> B, const int VERTEX, const LL MOD);
Matrix2D<LL> Power(Matrix2D<LL> A, const LL exp, const int VERTEX, const LL MOD);

int main() {
    cin.tie(nullptr);
    ios::sync_with_stdio(false);

    Matrix2D<LL> graph, res;
    ...
    graph = Matrix2D<LL>(V, V, 0);

    for (int e = 0; e < E; e++) {
        cin >> startV >> endV;
        InsertEdge(graph, startV - 1, endV - 1, WEIGHT);
        InsertEdge(graph, endV - 1, startV - 1, WEIGHT);
    }

    cin >> pathLength;

    res = Power(graph, pathLength, V, MOD);
    wayCount = res.m2[0][0];
...
Matrix2D<LL> Power(Matrix2D<LL> A, const LL exp, const int VERTEX, const LL MOD) {
    ...
    while (d > 0) {
        A = Multiplication(A, A, VERTEX, MOD);
        powers.push_back(A);
        d >>= 1;
    }
    ...
    for (size_t i = 0; i < size; i++) {
        if (exp & (1LL << i)) {
            res = Multiplication(res, powers[i], VERTEX, MOD);
        }
    }
