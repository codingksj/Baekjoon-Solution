...
bool CheckTile(int stR, int stC, int sz);
void Output();
...
void DFS(int r, int c, int sz) {
	++n;
	int half = sz >> 1;
	if (CheckTile(r, c, half)) {
		T[r + half - 1][c + half - 1] = n;
	}
	if (CheckTile(r, c + half, half)) {
		T[r + half - 1][c + half] = n;
	}
	if (CheckTile(r + half, c, half)) {
		T[r + half][c + half - 1] = n;
	}
	if (CheckTile(r + half, c + half, half)) {
		T[r + half][c + half] = n;
	}
	if (sz == 2) {
		return;
...
bool CheckTile(int stR, int stC, int sz) {
	for (int r = stR; r < stR + sz; r++) {
		for (int c = stC; c < stC + sz; c++) {
			if (T[r][c]) {
				return false;
			}
		}
