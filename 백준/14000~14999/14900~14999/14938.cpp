...
// ---------- 사용자 정의 함수 ----------
void InsertEdge(Matrix2D<int>& graph, const int v1, const int v2, const int w);
void FloydWarshall(Matrix2D<int>& dists, const int V);
int CalcMaxItemFarming(const Matrix2D<int>& dists, const vector<int>& items, const int V, const int limit);
...
int main() {
    ...
    dists = Matrix2D<int>(V, V, INF);
    items = InitVector<int>(V, 0);
    ...
    for (int i = 0; i < E; i++) {
        cin >> v1 >> v2 >> w;
        InsertEdge(dists, v1 - 1, v2 - 1, w);
        InsertEdge(dists, v2 - 1, v1 - 1, w);
    }

    FloydWarshall(dists, V);
    maxItem = CalcMaxItemFarming(dists, items, V, limit);
    ...
    for (int via = 0; via < V; via++) {
        for (int src = 0; src < V; src++) {
            for (int dest = 0; dest < V; dest++) {
                dists.m2[src][dest] = min(dists.m2[src][dest], dists.m2[src][via] + dists.m2[via][dest]);
            }
        }
    }
};
...
int CalcMaxItemFarming(const Matrix2D<int>& dists, const vector<int>& items, const int V, const int limit) {
    ...
    for (int src = 0; src < V; src++) {
        curItem = 0;
        for (int dest = 0; dest < V; (dists.m2[src][dest] <= limit) && (curItem += items[dest]), dest++) {}
