...
Pii Init(int node, int st, int ed) {
    ...
    int mid = (st + ed) / 2;
    return tree[node] = min(Init(2 * node, st, mid), Init(2 * node + 1, mid + 1, ed));
};

void Update(int node, int st, int ed, int idx, int val) {
    if (st == ed) {
        tree[node] = Pii(val, idx);
        return;
    }
    int mid = (st + ed) / 2;
    if (idx <= mid) {
...
Pii Query(int node, int st, int ed) {
    ...
    int mid = (st + ed) / 2;
    return min(Query(2 * node, st, mid), Query(2 * node + 1, mid + 1, ed));
};
