...
// ---------- 사용자 정의 함수 및 연산자 ----------
LL CountPossibleWay(const int R, const int C);
...
int main() {
    ...
    possibleWayCnt = CountPossibleWay(R, C);
...
LL CountPossibleWay(const int R, const int C) {
    Matrix2D<LL> dp = Matrix2D<LL>(R + 1, C + 1, 0);
    ...
    for (int r = 1; r <= R; r++) {
        for (int c = 1; c <= C; c++) {
            dp.m2[r][c] = (dp.m2[r][c] + dp.m2[r - 1][c] + dp.m2[r][c - 1] + dp.m2[r - 1][c - 1]) % MOD;
        }
    }
