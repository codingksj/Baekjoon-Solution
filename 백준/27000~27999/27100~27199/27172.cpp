...
// ---------- 사용자 정의 함수 ----------
vector<int> MakeScoreTable(const vector<int>& X);
void PrintScore(const vector<int>& X, const vector<int>& scoreX);
...
vector<int> MakeScoreTable(const vector<int>& X) {
    vector<int> scoreX = InitVector<int>(MILLION + 1, 0);
    vector<bool> existX = InitVector<bool>(MILLION + 1, false);
    ...
    for (const int& x : X) {
        for (int multiple = 2 * x; multiple <= MILLION; multiple += x) {
            if (existX[multiple]) {
                scoreX[x]++;
                scoreX[multiple]--;
            }
