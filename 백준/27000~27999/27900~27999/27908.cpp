...
void Solve() {
    lineCnt = (day + d + 5) / 7;
    calendar = Words(lineCnt + 2, bodyFormat);
    calendar[0] = calendar[lineCnt + 1] = edgeFormat;
    for (int n = 1; n <= day; n++) {
        lineNum = (n + d - 2) / 7 + 1;
        pos = (n + d - 2) % 7 ? (n + d - 2) % 7 : 0;
        pos = 3 * pos + 1;
        dayFormat = (n < 10 ? ".." : ".") + to_string(n);
        for (int i = pos; i <= pos + 2; i++) {
            calendar[lineNum][i] = dayFormat[i - pos];
        }
    }
};

void Output() {
    PrintVec<string>(calendar, "\n");
    return;
};
