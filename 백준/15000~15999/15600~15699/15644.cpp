...
bool Input();
void Solve();
Pii Move(Pii ball, bool& isOut);
bool Ok(const int r, const int c);
void Output();

// ---------- 전역 변수 ----------
queue<tuple<Pii, Pii, string>> ballQ;
Words board;
string minMove = "-1";
set<int> visChk;
Pii R, B, O;
...
bool Input() {
    ...
    board = InitVector<string>(N, string(M, '*'));
...
void Solve() {
    Pii nRed, nBlue;
    bool isOutRed, isOutBlue;

    ballQ.push(tuple(R, B, ""));

    while (!ballQ.empty()) {
        auto [red, blue, mv] = ballQ.front(); ballQ.pop();
        auto& [redR, redC] = red;
        auto& [blueR, blueC] = blue;

        if (mv.length() >= MAX_MOVE) {
            continue;
        }
        for (int i = 0; i < 4;i++) {
            isOutRed = false;
            ...
            if ((redR - blueR) * dr < 0 || (redC - blueC) * dc < 0) {
                nBlue = Move(blue, isOutBlue);
                nRed = Move(red, isOutRed);
            }
            else {
                nRed = Move(red, isOutRed);
                nBlue = Move(blue, isOutBlue);
            ...
            board[nRed.first][nRed.second] = EMPTY;
            board[nBlue.first][nBlue.second] = EMPTY;

            if (isOutBlue) {
                continue;
            }
            if (isOutRed) {
                minMove = (mv + MOVEMENT[i]);
                return;
            }

            int key = 100 * (nRed.first * M + nRed.second) + (nBlue.first * M + nBlue.second);
            ...
            if (visChk.count(key)) {
                ...
                visChk.insert(key);
                ballQ.push(tuple(nRed, nBlue, mv + MOVEMENT[i]));
            }
        }
    }
}
...
Pii Move(Pii ball, bool& isOut) {
    ...
    board[O.first][O.second] = HOLE;

    for (nr = r + dr, nc = c + dc; Ok(nr, nc); nr += dr, nc += dc) {
        auto obj = board[nr][nc];
        if (obj == WALL || obj == RED || obj == BLUE) {
            nr -= dr;
            nc -= dc;
            break;
        }
