...
void Solve() {
    for (int group = 1; group < (1 << N); group++) {
        minDiff = min(minDiff, CalcScore(group));
    }
};
...
int CalcScore(const int group) {
    ...
    for (int p = 0; p < N; p++) {
        (group & (1 << p)) ? team1.push_back(p) : team2.push_back(p);
    }
    sz1 = team1.size();
    sz2 = team2.size();
    ...
    return (sz1 * sz2 ? abs(s1 - s2) : BILLION);
};
