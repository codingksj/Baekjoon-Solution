...
typedef struct BigInt {
    ...
    BigInt operator+ (const BigInt& other) {
        BigInt res;
        int digit1, digit2, carry = 0, sum;
        int maxL;

        maxL = max(length, other.length);
        res.number = string(maxL, '0');

        for (int i = 0; i < maxL; i++) {
            digit1 = (i < length) ? number[length - 1 - i] - '0' : 0;
            digit2 = (i < other.length) ? other.number[other.length - 1 - i] - '0' : 0;

            sum = digit1 + digit2 + carry;
            carry = sum / 10;
            res.number[maxL - 1 - i] = (sum % 10) + '0';
        }
