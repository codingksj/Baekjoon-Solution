...
//사용자 정의 함수
Pii CalcMatch(const string& me, const string& her, const vector<int>& alpha);
...
Pii CalcMatch(const string& me, const string& her, const vector<int>& alpha) {
    vector<int> digits, newDigits;
    Pii match;
    int maxLength = me.length();
    int iM, iH;
    int size;
    int value;
    ...
    for (int i = 0; i < maxLength; i++) {
        iM = me[i] - 'A';
        iH = her[i] - 'A';
        digits.push_back(alpha[iM]);
        digits.push_back(alpha[iH]);
    }
