...
// ---------- 사용자 정의 함수 및 연산자 ----------
string CheckLinear(const vector<Pii>& coords);
...
int main() {
    ...
    vector<Pii> coords = LoadVector<int, int>(3);

    cout << CheckLinear(coords) << "\n";
...
string CheckLinear(const vector<Pii>& coords) {
    ...
    delta1 = coords[1] - coords[0];
    delta2 = coords[2] - coords[1];

    return (delta1.first * delta2.second == delta2.first * delta1.second ? "WHERE IS MY CHICKEN?" : "WINNER WINNER CHICKEN DINNER!");
};
