...
//사용자 지정 함수
int CalcMinEdit(const string& s, const string& t);
...
int CalcMinEdit(const string& s, const string& t) {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= m; j++) {
            changeCost = (s[i - 1] == t[j - 1]) ? 0 : 1;
            d[i][j] = min({ d[i][j - 1] + 1, d[i - 1][j] + 1, d[i - 1][j - 1] + changeCost });
        }
    }
