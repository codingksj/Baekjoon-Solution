...
// ---------- 전역 변수 ----------
Vi counts, cards, uniques;
string combination;
int TC, pairCnt, tripleCnt, quadCnt;
...
void Solve() {
    counts = Vi(14);
    uniques = Vi();
    quadCnt = tripleCnt = pairCnt = 0;

    for (auto& card : cards) {
        counts[card]++;
    }
    for (int i = 1; i <= 13; i++) {
        counts[i] > 3 ? quadCnt++ : counts[i] > 2 ? tripleCnt++ : counts[i] > 1 ? pairCnt++ : 0;
    }
    if (quadCnt) {
        combination = pairCnt ? "tiki pair" : "tiki";
    }
    else if (tripleCnt) {
        combination = (tripleCnt == 2) ? "two triples" : (pairCnt ? "full house" : "one triple");
    }
    else if(pairCnt){
        combination = (pairCnt == 3) ? "three pairs" : (pairCnt == 2 ? "two pairs" : "one pair");
    }
    else {
        combination = "single";
    }
