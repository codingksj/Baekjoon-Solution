...
bool Input();
void Solve();
void DFS(const int v, const int mxW, const int totalW);
void Output();
...
bool Input() {
    ...
    graph = Init2DVector<Pii>(N + 1, 0, Pii());
    visChk = Vb(N + 1, false);

    for (int i = 0; i < N - 1; i++) {
        cin >> v1 >> v2 >> w;
        
        graph[v1].push_back(Pii(v2, w));
        graph[v2].push_back(Pii(v1, w));
    }
