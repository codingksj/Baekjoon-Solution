...
// ---------- 전역 변수 ----------
Vi tbA = { 5'000'000, 3'000'000, 2'000'000, 500'000, 300'000, 100'000, 0 };
Vi tbB = { 5'120'000, 2'560'000, 1'280'000, 640'000, 320'000, 0 };
int TC, n, a, b, iA, iB;
int prizeA, prizeB;
...
void Solve() {
    for (n = 1, iA = 1, prizeA = 0; n < 22; n += ++iA) {
        if (a > 0 && a <= n) {
            prizeA = tbA[iA - 1];
            break;
        }
    }
    for (n = 2, iB = 0, prizeB = 0; n < (1 << 6); n <<= 1, ++iB) {
        if (b > 0 && b < n) {
            prizeB = tbB[iB];
            break;
        }
