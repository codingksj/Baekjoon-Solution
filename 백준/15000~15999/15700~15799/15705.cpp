...
//사용자 지정 함수
bool FindTargetFromDict(const Words& words, const string& target, const int R, const int C);
...
int main() {
    ...
    words = LoadVector<string>(R);
    hasTarget = FindTargetFromDict(words, target, R, C);
        ...
        for (int c = 0; c < C; c++) {
            for (int i = 0; i < 8; i++) {
                for (length = 0; length < targetLength; length++) {
                    ...
                    isMatch = (target[length] == words[nr][nc]);
                    if (!isMatch) {
                        break;
                    }
                }
