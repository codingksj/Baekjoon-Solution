...
bool CheckCanDivideIntoPrimeLength(const vector<LL>& primes, const LL A, const LL B);
bool CheckPrime(const vector<LL>& primes, const LL N);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<LL> primes = LoadPrimes(2 * MILLION);
    LL A, B;
    int TC;
    bool canDivide;
    ...
    while (TC--) {
        ...
        canDivide = CheckCanDivideIntoPrimeLength(primes, A, B);

        cout << (canDivide ? "YES" : "NO") << "\n";
    }
...
bool CheckCanDivideIntoPrimeLength(const vector<LL>& primes, const LL A, const LL B) {
    LL sum = A + B;
    bool canDivide = (sum & 1 ? CheckPrime(primes, sum - 2) : sum > 2);
...
vector<LL> LoadPrimes(const LL N) {
    vector<LL> primes;
    vector<bool> isPrime = InitVector<bool>(N + 1, true);
