...
void Solve() {
	...
	auto [_0, invFnr, _1] = ExtGCD(fNR, MOD);
	auto [_2, invFr, _3] = ExtGCD(fR, MOD);
	invFnr = (invFnr < 0) ? invFnr + MOD : invFnr;
	invFr = (invFr < 0) ? invFr + MOD : invFr;
	res = fN * invFnr % MOD;
	res = res * invFr % MOD;
	return;
};
