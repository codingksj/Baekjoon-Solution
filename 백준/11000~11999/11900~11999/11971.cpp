...
bool Input() {
	...
	limitSpeeds = LoadVec<int, int>(N);
	mySpeeds = LoadVec<int, int>(M);

	return true;
};

void Solve() {
	for (int ptr1 = 0, ptr2 = 0; ptr1 < N && ptr2 < M;) {
		auto& [len, spd] = limitSpeeds[ptr1];
		auto& [myLen, mySpd] = mySpeeds[ptr2];
		if (mySpd > spd) {
			mxExceed = max(mxExceed, mySpd - spd);
		}
		int used = min(len, myLen);
		len -= used;
		myLen -= used;
		if (!len) {
			ptr1++;
