...
int main() {
    ...
    if (isCycle) {
        ...
        for (int i = 2; i <= V; i++) {
            cout << (dists[i] == BILLION ? -1 : dists[i]) << "\n";
        }
    }
...
void BellmanFord() {
    isCycle = false;
    dists = InitVector<LL>(V + 1, BILLION);
    dists[startV] = 0;

    for (int level = 1; level < V; level++) {
        for (const auto& [via, dest, w] : graph) {
            if (dists[via] != BILLION && dists[via] + w < dists[dest]) {
                dists[dest] = dists[via] + w;
            }
        }
    }
    for (const auto& [via, dest, w] : graph) {
