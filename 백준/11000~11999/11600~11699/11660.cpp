...
//사용자 지정 함수
Matrix2D_i Make2DPrefixSum(const Matrix2D_i& V, const int N);
void DoQuery(const Matrix2D_i& pfV, const int Q);
...
int main() {
    ...
    V = Load2DVector<int>(N, N);
    pfV = Make2DPrefixSum(V, N);
...
Matrix2D_i Make2DPrefixSum(const Matrix2D_i& V, const int N) {
    Matrix2D_i pfV = Init2DVector<int>(N + 1, N + 1, 0);
...
void DoQuery(const Matrix2D_i& pfV, const int Q) {
    ...
    for (int i = 0; i < Q; i++) {
        cin >> x1 >> y1 >> x2 >> y2;
        sum = pfV[x2][y2] - pfV[x2][y1 - 1] - pfV[x1 - 1][y2] + pfV[x1 - 1][y1 - 1];
        cout << sum << "\n";
    }
};
