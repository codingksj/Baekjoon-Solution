...
// ---------- 사용자 정의 함수 및 연산자 ----------
LL RangeMul(const LL start, const LL end, const LL MOD);
LL Power(const LL base, const LL exp, const LL MOD);
...
int main() {
    ...
    top = RangeMul(N - K + 1, N, MOD);
    bottom = Power(RangeMul(1, K, MOD), MOD - 2, MOD);
    res = top * bottom % MOD;
