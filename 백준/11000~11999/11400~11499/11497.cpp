...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CalcMinDiff(const Vi& logs, const int N);
...
int main() {
    ...
    while (TC--) {
        cin >> N;
        logs = LoadVector<int>(N);
        SortAll(logs);
        cout << CalcMinDiff(logs, N) << "\n";
    }
...
int CalcMinDiff(const Vi& logs, const int N) {
    Vi best = InitVector<int>(N, 0);
    int minDiff = 0;

    for (int i = 0, left = 0, right = N - 1; i < N; i++) {
        (i & 1) ? (best[left++] = logs[i]) : (best[right--] = logs[i]);
    }
    for (int i = 0; i < N; i++) {
        minDiff = max(minDiff, abs(best[i % N] - best[(i + 1) % N]));
    }
