...
//사용자 지정 함수
void UpdateEdge(Matrix2D_i& costs, const int vSrc, const int vDest, const int cost);
void FloydWarshall(Matrix2D_i& costs, const int V);
void PrintCosts(Matrix2D_i& costs, const int V);
...
int main() {
    ...
    costs = Init2DVector<int>(V + 1, V + 1, BILLION);
...
void UpdateEdge(Matrix2D_i& costs, const int vSrc, const int vDest, const int cost) {
    costs[vSrc][vDest] = min(costs[vSrc][vDest], cost);
};
    ...
    for (int pass = 1; pass <= V; pass++) {
        for (int src = 1; src <= V; src++) {
            for (int dest = 1; dest <= V; dest++) {
                costs[src][dest] = min(costs[src][dest], costs[src][pass] + costs[pass][dest]);
            }
        }
    }
};
