...
//사용자 지정 함수
Matrix2D_i MakeGraph(const int V);
void FloydWarshall(Matrix2D_i& graph, const int V);
void PrintGraph(Matrix2D_i& graph, const int V);
...
Matrix2D_i MakeGraph(const int V) {
    Matrix2D_i graph = Init2DVector<int>(V, V, BILLION);
    int e;
    ...
    for (int pass = 0; pass < V; pass++) {
        for (int src = 0; src < V; src++) {
            for (int dest = 0; dest < V; dest++) {
                graph[src][dest] = min(graph[src][dest], graph[src][pass] + graph[pass][dest]);
            }
        }
    }
};
