...
// ---------- 사용자 정의 함수 ----------
void InsertEdge(Matrix2D<int>& graph, const int v1, const int v2);
vector<int> BFS(const Matrix2D<int>& graph, const int V, const int start);
void PrintParents(const vector<int>& parents, const int V);
...
int main() {
    ...
    parents = BFS(graph, V, startV);

    PrintParents(parents, V);

    return 0;
}

void InsertEdge(Matrix2D<int>& graph, const int v1, const int v2) {
    graph.m2[v1].push_back(v2);
};

vector<int> BFS(const Matrix2D<int>& graph, const int V, const int start) {
    queue<int> q;
    vector<int> parents = InitVector<int>(V + 1, 0);
    vector<bool> bfsCheck = InitVector<bool>(V + 1, false);
    int parent;
    ...
    while (!q.empty()) {
        ...
        for (const int& child : graph.m2[parent]) {
            if (!bfsCheck[child]) {
