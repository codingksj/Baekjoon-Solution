...
bool Input() {
    ...
    S = Init2DVector<int>(N, N, INF);
    R = Init2DVector<int>(N, N, -1);
    ...
    for (int i = 0; i < M; i++) {
        ...
        S[vSrc][vDest] = min(S[vSrc][vDest], cost);
        R[vSrc][vDest] = vDest;
    }
    ...
    for (int via = 0; via < N; via++) {
        for (int src = 0; src < N; src++) {
            for (int dest = 0; dest < N; dest++) {
                ...
                if (S[src][dest] > S[src][via] + S[via][dest]) {
                    S[src][dest] = S[src][via] + S[via][dest];
                    R[src][dest] = R[src][via];
                }
            }
        }
    }
...
void Output() {
    for (int src = 0; src < N; src++) {
        for (int dest = 0; dest < N; dest++) {
            cout << (S[src][dest] == INF ? 0 : S[src][dest]) << " \n"[dest == N - 1];
        }
    }
    for (int src = 0; src < N; src++) {
        for (int dest = 0; dest < N; dest++) {
            ...
            for (int now = src;; now = R[now][dest]) {
