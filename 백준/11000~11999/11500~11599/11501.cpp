...
//사용자 지정 함수
LL CalcMaxProfit(const vector<LL>& stocks, const LL N);
...
int main() {
    ...
    while (TC--) {
        ...
        stocks = LoadVector<LL>(N);
        maxProfit = CalcMaxProfit(stocks, N);
...
LL CalcMaxProfit(const vector<LL>& stocks, const LL N) {
    ...
    for (int i = N - 1; i >= 0; i--) {
        (high < stocks[i]) ? (high = stocks[i]) : (maxProfit += high - stocks[i]);
    }
