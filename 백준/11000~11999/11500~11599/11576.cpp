...
// ---------- 사용자 정의 함수 ----------
vector<int> ConvertBase(const vector<int>& digitA, const int A, const int B);
...
int main() {
    ...
    digitA = LoadVector<int>(M);
    digitB = ConvertBase(digitA, A, B);

    PrintVector<int>(digitB, " ");
...
vector<int> ConvertBase(const vector<int>& digitA, const int A, const int B) {
    ...
    for (const int& digit : digitA) {
        decimal = decimal * A + digit;
    }

    while (decimal > 0) {
        digitB.push_back(decimal % B);
        decimal /= B;
    }
