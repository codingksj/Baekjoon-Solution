...
const Words STATES = { "NEITHER", "INCREASING", "DECREASING" };
...
bool Input() {
    ...
    players = LoadVector<string>(N);

    return true;
};

void Solve() {
    isUp = is_sorted(players.begin(), players.end(), less<string>());
    isDown = is_sorted(players.begin(), players.end(), greater<string>());
};

void Output() {
    cout << STATES[isUp + 2 * isDown] << "\n";
};
