...
LL Init(int node, int st, int ed) {
    ...
    int mid = (st + ed) / 2;
    LL leftChild = Init(2 * node, st, mid);
    LL rightChild = Init(2 * node + 1, mid + 1, ed);
...
void Update(int node, int st, int ed) {
    if (st == ed) {
        ...
        int mid = (st + ed) / 2;
		idx <= mid ? Update(2 * node, st, mid) : Update(2 * node + 1, mid + 1, ed);
...
LL Query(int node, int st, int ed) {
    ...
    int mid = (st + ed) / 2;
    LL leftMul = Query(2 *node, st, mid);
    LL rightMul = Query(2 * node + 1, mid + 1, ed);
