...
// ---------- 전역 변수 ----------
vector<int> line1, line2, dp1, dp2;
int TC, N;
...
int main() {
    ...
    while (TC--) {
        ...
        line1 = LoadVector<int>(N);
        line2 = LoadVector<int>(N);
        dp1 = InitVector<int>(N + 1, 0);
        dp2 = InitVector<int>(N + 1, 0);
...
int CalcMaxScore() {
    ...
    for (int i = 2; i <= N; i++) {
        dp1[i] = max(dp2[i - 1], dp2[i - 2]) + line1[i - 1];
        dp2[i] = max(dp1[i - 1], dp1[i - 2]) + line2[i - 1];
    }
