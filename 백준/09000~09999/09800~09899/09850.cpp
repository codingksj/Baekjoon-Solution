...
const string WORD1 = "CHIPMUNKS";
const string WORD2 = "LIVE";
...
bool Input();
void Solve();
string Convert(const string& cipher, int P);
void Output();

// ---------- 전역 변수 ----------
string cipher, plainTxt;
...
void Solve() {
	for (int i = 0; i < ALPHABETS; i++) {
		plainTxt = Convert(cipher, i);
		if (plainTxt.contains(WORD1) && plainTxt.contains(WORD2)) {
			return;
		}
	}
};
...
string Convert(const string& cipher, int P) {
	...
	for (const auto& ch : cipher) {
		res += isalpha(ch) ? 'A' + (ch - 'A' + P) % ALPHABETS : ch;
	}
	return res;
};
