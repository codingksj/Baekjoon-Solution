...
bool Input();
void Solve();
Pii Init(int node, int st, int ed);
Pii Update(int node, int st, int ed, int idx);
Pii Query(int node, int st, int ed);
void Output();
...
void Solve() { 
    tree = InitVec<Pii>(4 * N, Pii(INF, -1));
    dvd = InitArithmeticSeq<int>(N + 1, 0);
    ...
    for (int i = 0; i < K; i++) {
        ...
        if (q == 0) {
            ...
            ++l, ++r;
			res = Query(ROOT, 1, N);
            cout << ((res.first >= l && res.second <= r) ? "YES\n" : "NO\n");
        }
    }
}
...
Pii Init(int node, int st, int ed) {
    if (st == ed) {
        return tree[node] = Pii(dvd[st], dvd[ed]);
    }

    int mid = (st + ed) / 2;
    Pii leftChild = Init(2 * node, st, mid);
    Pii rightChild = Init(2 * node + 1, mid + 1, ed);

    return tree[node] = Pii(min(leftChild.first, rightChild.first),
        max(leftChild.second, rightChild.second));
};

Pii Update(int node, int st, int ed, int idx) {
    if (st == ed) {
        return tree[node] = Pii(dvd[idx], dvd[idx]);
    }
    int mid = (st + ed) / 2;
    Pii leftChild, rightChild;

    if (idx <= mid) {
        leftChild = Update(2 * node, st, mid, idx);
        rightChild = tree[2 * node + 1];
    }
    else {
        leftChild = tree[2 * node];
        rightChild = Update(2 * node + 1, mid + 1, ed, idx);
    }
...
Pii Query(int node, int st, int ed) {
    ...
    int mid = (st + ed) / 2;
    Pii leftQ = Query(2 *node, st, mid);
    Pii rightQ = Query(2 * node + 1, mid + 1, ed);

    return Pii(min(leftQ.first, rightQ.first),
        max(leftQ.second, rightQ.second));
};
