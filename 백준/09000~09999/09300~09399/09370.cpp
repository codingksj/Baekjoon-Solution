...
void Dijkstra(vector<int>& dists, const int startV);
void PrintTarget();
...
int main() {
    ...
    while (TC--) {
        ...
        dests = LoadVector<int>(destCount);
...
void Dijkstra(vector<int>& dists, const int startV) {
    ...
    dists = InitVector<int>(V + 1, 51 * MILLION);
    pq.push({ 0, startV });
...
void PrintTarget() {
    for (const int& dest : dests) {
        isGH = distS[dest] == (distS[g] + distG[h] + distH[dest]);
        isHG = distS[dest] == (distS[h] + distH[g] + distG[dest]);
        if (isGH || isHG) {
            cout << dest << " ";
        }
    }
