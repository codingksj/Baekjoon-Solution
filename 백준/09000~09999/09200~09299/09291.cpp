...
typedef struct Sudoku {
    ...
    Matrix2D_i rowsInfo;
    Matrix2D_i colsInfo;
    Matrix2D_i squaresInfo;
    int size;
}Sudoku;

//사용자 지정 함수
Sudoku InitSudoku(const int size);
Matrix2D_i FillRowsInfo(const Matrix2D_i& board, const int size);
Matrix2D_i FillColsInfo(const Matrix2D_i& board, const int size);
Matrix2D_i FillSquaresInfo(const Matrix2D_i& board, const int size);
bool CheckSudoku(const Sudoku& sudoku);
bool CheckArea(const vector<int>& info, const int size);
...
int main() {
    ...
    for (int i = 1; i <= TC; i++) {
        sudoku = InitSudoku(size);
        isValid = CheckSudoku(sudoku);
...
Sudoku InitSudoku(const int size) {
    ...
    sudoku.board = Load2DVector<int>(size, size);
    sudoku.rowsInfo = FillRowsInfo(sudoku.board, size);
    sudoku.colsInfo = FillColsInfo(sudoku.board, size);
    sudoku.squaresInfo = FillSquaresInfo(sudoku.board, size);
    sudoku.size = size;
...
Matrix2D_i FillRowsInfo(const Matrix2D_i& board, const int size) {
    Matrix2D_i rowsInfo = Init2DVector<int>(size, size + 1, 0);

    for (int r = 0; r < size; r++) {
        for (int c = 0; c < size; c++) {
            rowsInfo[r][board[r][c]]++;
        }
    }
    return rowsInfo;
};
...
Matrix2D_i FillColsInfo(const Matrix2D_i& board, const int size) {
    Matrix2D_i colsInfo = Init2DVector<int>(size, size + 1, 0);

    for (int r = 0; r < size; r++) {
        for (int c = 0; c < size; c++) {
            colsInfo[c][board[r][c]]++;
        }
    }
    return colsInfo;
};
...
Matrix2D_i FillSquaresInfo(const Matrix2D_i& board, const int size) {
    Matrix2D_i squaresInfo = Init2DVector<int>(size, size + 1, 0);
    int index;

    for (int r = 0; r < size; r++) {
        for (int c = 0; c < size; c++) {
            index = (r / 3) * 3 + c / 3;
            squaresInfo[index][board[r][c]]++;
        }
    }
    return squaresInfo;
};
...
bool CheckSudoku(const Sudoku& sudoku) {
    ...
    for (int i = 0; i < size; i++) {
        isValid = CheckArea(sudoku.rowsInfo[i], size) &&
            CheckArea(sudoku.colsInfo[i], size) &&
            CheckArea(sudoku.squaresInfo[i], size);
        if (!isValid) {
            return false;
        }
    }
