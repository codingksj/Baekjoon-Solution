...
// ---------- 사용자 정의 함수 ----------
int CalcFastestInvite(const vector<int>& trees, const int N);
...
int main() {
    ...
    trees = LoadVector<int>(N);
    Sort(trees, greater<int>());
    minDay = CalcFastestInvite(trees, N);
