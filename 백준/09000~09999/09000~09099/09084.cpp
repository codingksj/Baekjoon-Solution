...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CountPossibleWay(const vector<int>& coins, const int N, const int target);
...
int main() {
    ...
    while (TC--) {
        ...
        wayCount = CountPossibleWay(coins, N, target);
...
int CountPossibleWay(const vector<int>& coins, const int N, const int target) {
    vector<int> dp = InitVector<int>(target + 1, 0);
    dp[0] = 1;
