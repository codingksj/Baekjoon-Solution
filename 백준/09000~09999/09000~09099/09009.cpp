...
//사용자 지정 함수
vector<int> LoadFibonaccis(const int N);
vector<int> FindMinFibonaccisForTarget(const vector<int>& fibonaccis, const int target);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<int> fibonaccis = LoadFibonaccis(BILLION);
    vector<int> minFibonaccis;
    int TC, target;

    cin >> TC;

    Sort(fibonaccis, greater<int>());

    while (TC--) {
        cin >> target;

        minFibonaccis = FindMinFibonaccisForTarget(fibonaccis, target);

        PrintVector<int>(minFibonaccis, " ");
...
vector<int> LoadFibonaccis(const int N) {
    vector<int> fibos = { 0,1,1 };
    int fibo;

    for (int i = 2; fibos[i] < N; i++) {
        fibo = fibos[i - 1] + fibos[i];
...
vector<int> FindMinFibonaccisForTarget(const vector<int>& fibonaccis, const int target) {
    ...
    for (const int& fibo : fibonaccis) {
        if (!total) {
            break;
        }
        if (total >= fibo) {
            total -= fibo;
            minFibonaccis.push_back(fibo);
        }
    }

    Sort(minFibonaccis);

    return minFibonaccis;
};