...
Pii FindGoldbachPartition(const vector<bool>& primes, const int N);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<bool> primeChecks = LoadPrimeChecks(MAX);
    Pii partition;
    int TC, N;
    ...
    while (TC--) {
        ...
        partition = FindGoldbachPartition(primeChecks, N);
...
Pii FindGoldbachPartition(const vector<bool>& primeChecks, const int N) {
    Pii partition = { -1,-1 };
    int p1, p2;

    for (p1 = N / 2; p1 >= 2; p1--) {
        p2 = N - p1;
        if (primeChecks[p1] && primeChecks[p2]) {
            partition = { p1,p2 };
            break;
        }
    }
