...
bool Input() {
	times = LoadVec<string>(MAX);
	return true;
};

void Solve() {
	ranges::sort(times, Compare);
	return;
};

int GetAngle(const string& s) {
	int h = (10 * (s[0] - '0') + (s[1] - '0')) % 12;
	int m = 10 * (s[3] - '0') + (s[4] - '0');
	int angle = abs(60 * h - 11 * m);
	return min(angle, 720 - angle);
};

bool Compare(const string& s1, const string& s2) {
	int angle1 = GetAngle(s1);
	int angle2 = GetAngle(s2);
	if (angle1 != angle2) {
		return GetAngle(s1) < GetAngle(s2);
	}
	return s1 < s2;
};
