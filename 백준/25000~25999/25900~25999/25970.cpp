...
void Solve() {
	...
	while (N--) {
		...
		int sz = realData.size();
		int delta = 0;

		for (int i = 0; i < sz; i++) {
			LL val = 0;
			for (int j = i; j < min(i + 50, sz); j++) {
				val = (val << 1) | (realData[j] & 1);
				auto it = mp.find(val | saltTable[j - i + 1]);
				if (it != mp.end()) {
					delta += it->second;
				}
			}
