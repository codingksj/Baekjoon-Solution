...
//사용자 지정 함수
int BalancingGravel(const int N);
int CountMinWeight(const int delta);
...
int main() {
    ...
    delta = BalancingGravel(N);
    count = CountMinWeight(delta);
...
int CountMinWeight(const int delta) {
    const vector<int> weights = { 100, 50, 20, 10, 5, 2, 1 };
    int d = delta;
    int N, count = 0;
