...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CountMinSquare(const Words& board, const Tiii& sizes);
...
int main() {
    ...
    board = LoadVector<string>(N);

    cout << CountMinSquare(board, Tiii(N, M, K)) << "\n";
...
int CountMinSquare(const Words& board, const Tiii& sizes) {
    ...
    int minChange, val;
    int curChange;
    const auto& [N, M, K] = sizes;

    pSum = Init2DVector<int>(N + 1, M + 1, 0);
    minChange = K * K;

    for (int r = 0; r < N; r++) {
        for (int c = 0; c < M; c++) {
            val = (board[r][c] == ((r + c) & 1 ? 'B' : 'W'));
    ...
    for (int r = 0; r <= N - K; r++) {
        for (int c = 0; c <= M - K; c++) {
            curChange = pSum[r + K][c + K] - (pSum[r][c + K] + pSum[r + K][c]) + pSum[r][c];
            minChange = min({ minChange, curChange, K * K - curChange });
        }
    }
