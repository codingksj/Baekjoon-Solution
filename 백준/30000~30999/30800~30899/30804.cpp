...
bool Input();
void Solve();
int CalcDiff(const Vi& A, const Vi& B);
void Output();
...
bool Input() {
    cin >> N;
    fruitCounters = VVi(N + 1, Vi(SIZE, 0));
    for (int i = 0; i < N; i++) {
        fruitCounters[i + 1] = fruitCounters[i];
        cin >> fruit;
        fruitCounters[i + 1][fruit]++;
    }
...
void Solve() {
    ...
    for (int ptrL = 1, ptrR = ptrL + 1; ptrL < ptrR && ptrR <= N;) {
        int diff = CalcDiff(fruitCounters[ptrL - 1], fruitCounters[ptrR]);
        if (diff <= 2) {
            mxT = max(mxT, ptrR - ptrL + 1);
            ptrR++;
        }
        else {
            ptrL++;
