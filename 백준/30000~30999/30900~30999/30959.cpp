...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CalcSingleAccuracy(const vector<int>& model);
void FindMaxSingleModel();
int CalcEnsembleAccuracy();
void DFS(const int depth, const int startIndex);
...
int main() {
    ...
    ans = LoadVector<int>(M);
    dfsCheck = InitVector<bool>(N, false);
    models.m2 = Load2DVector<int>(N, M);

    FindMaxSingleModel();

    for (maxModelCount = 3; maxModelCount <= N; maxModelCount += 2) {
        indexes = InitVector<int>(maxModelCount, 0);
        DFS(0, 0);
    }
...
void FindMaxSingleModel() {
    ...
    for (const vector<int>& model : models.m2) {
        curSingle = CalcSingleAccuracy(model);
        maxSingle = max(maxSingle, curSingle);
    }
};

int CalcEnsembleAccuracy() {
    vector<int> ensembleModel(M);

    for (const int& index : indexes) {
        for (int i = 0; i < M; i++) {
            ensembleModel[i] += models.m2[index][i];
        }
    }
    for (int i = 0; i < M; i++) {
        ensembleModel[i] = ensembleModel[i] > maxModelCount / 2;
    }

    return CalcSingleAccuracy(ensembleModel);
};
...
void DFS(const int depth, const int startIndex) {
    ...
    if (depth == maxModelCount) {
        curMulti = CalcEnsembleAccuracy();
        if (curMulti > maxSingle) {
            isExist = true;
        }
        return;
    }
