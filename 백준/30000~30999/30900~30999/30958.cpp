...
// ---------- 사용자 정의 함수 및 연산자 ----------
int FindMostAppearance(const string& song);
...
int FindMostAppearance(const string& song) {
    vector<int> alphaCounts = InitVector<int>(26, 0);

    for (const char& ch : song) {
        if (isalpha(ch)) {
            alphaCounts[ch - 'a']++;
        }
    }

    return *max_element(alphaCounts.begin(), alphaCounts.end());
};