...
// ---------- 사용자 정의 함수 및 연산자 ----------
LL Intergral(const int x1, const int x2, const vector<LL>& coeffs);
LL F(const int x, const vector<LL>& coeffs);
LL Pow(const int x, const int exp);
...
int main() {
    ...
    LL s1, s2;
    vector<LL> coeffsF, coeffsG;
    int x1, x2;

    cin >> x1 >> x2;

    coeffsF = LoadVector<LL>(3);
    coeffsG = LoadVector<LL>(2);
    s1 = Intergral(x1, x2, coeffsF);
    s2 = Intergral(x1, x2, coeffsG);
...
LL Intergral(const int x1, const int x2, const vector<LL>& coeffs) {
    return F(x2, coeffs) - F(x1, coeffs);
};
