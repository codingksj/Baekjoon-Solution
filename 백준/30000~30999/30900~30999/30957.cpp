...
// ---------- 사용자 정의 함수 및 연산자 ----------
string FindMostInterest(const string& Q, const int N);
...
string FindMostInterest(const string& Q, const int N) {
    ...
    for (const char& curSubject : Q) {
        for (auto& [count, subject] : counts) {
    ...
    maxCount = max({ counts[0].first, counts[1].first, counts[2].first });
