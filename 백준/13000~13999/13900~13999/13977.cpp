...
bool Input();
void Solve();
LL ModPow(const LL base, const LL exp, const LL MOD);
void Output();
...
void Solve() {
	while(M--){
		cin >> N >> K;
		res = factMod[N] * ModPow(factMod[K] * factMod[N - K] % MOD, MOD - 2, MOD) % MOD;
		cout << res << "\n";
	}
	return;
};
