...
// ---------- 사용자 정의 함수 및 연산자 ----------
Pii FindClosestTaxi(const vector<Pii>& taxis, const Pii& me);
...
int main() {
    ...
    taxis = LoadVector<int, int>(N);
    closestTaxi = FindClosestTaxi(taxis, me);

    PrintPair<int, int>(closestTaxi, " ");
