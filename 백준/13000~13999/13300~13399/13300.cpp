...
//사용자 지정 함수
vector<Grade> LoadStudents(const LL N);
int CountRoom(const vector<Grade>& students, const int K);
...
vector<Grade> LoadStudents(const LL N) {
    vector<Grade> students = InitVector<Grade>(7, { 0,0 });
    int sex, grade;

    for (int i = 0; i < N; i++) {
        cin >> sex >> grade;
        sex ? students[grade].boy++ : students[grade].girl++;
    }
...
int CountRoom(const vector<Grade>& students, const int K) {
    ...
    for (int i = 1; i <= 6; i++) {
        room += ceil((double)students[i].boy / K);
        room += ceil((double)students[i].girl / K);
    }
