...
//사용자 지정 함수
int BFS(vector<bool>& bfsCheck, const int start, const int target);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<bool> bfsCheck = InitVector<bool>(MAX + 1, false);
    int me, brother;
    int minTime;

    cin >> me >> brother;

    minTime = BFS(bfsCheck, me, brother);
...
int BFS(vector<bool>& bfsCheck, const int start, const int target) {
    ...
    while (!pq.empty()) {
        ...
        if (x1 >= 0 && x1 <= MAX && !bfsCheck[x1]) {
            pq.push({ time, x1 });
            bfsCheck[x1] = true;
        }
        if (x2 >= 0 && x2 <= MAX && !bfsCheck[x2]) {
            pq.push({ time + 1, x2 });
            bfsCheck[x2] = true;
        }
        if (x3 >= 0 && x3 <= MAX && !bfsCheck[x3]) {
            pq.push({ time + 1, x3 });
            bfsCheck[x3] = true;
        }
    }
