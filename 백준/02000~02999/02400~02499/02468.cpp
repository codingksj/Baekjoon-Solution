...
Matrix2D_b MakeBFSCheck(const Matrix2D_i& area, const Pii& limit, const int height);
int FindMaxSafeZone(const Matrix2D_i& area, Matrix2D_b& bfsCheck, const Pii& limit);
void BFS(const Matrix2D_i& area, Matrix2D_b& bfsCheck, const Pii& limit, const Pii& start);
...
int main() {
    ...
    limit = { N,N };
    area = Load2DVector<int>(N, N);
    bfsCheck = MakeBFSCheck(area, limit, 0);
    maxSafeZone = FindMaxSafeZone(area, bfsCheck, limit);
...
int FindMaxSafeZone(const Matrix2D_i& area, Matrix2D_b& bfsCheck, const Pii& limit) {
    ...
    for (int h = 0; h <= 100; h++) {
        bfsCheck = MakeBFSCheck(area, limit, h);
        curSafeZone = 0;
        for (int r = 0; r < R; r++) {
            for (int c = 0; c < C; c++) {
                if (!bfsCheck[r][c]) {
                    start = { r,c };
                    BFS(area, bfsCheck, limit, start);
                    curSafeZone++;
                }
            }
        }
        maxSafeZone = max(maxSafeZone, curSafeZone);
    }
...
void BFS(const Matrix2D_i& area, Matrix2D_b& bfsCheck, const Pii& limit, const Pii& start) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (isValidCoord && !bfsCheck[nr][nc]) {
                bfsCheck[nr][nc] = true;
                q.push({ nr,nc });
            }
        }
