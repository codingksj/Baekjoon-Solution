...
void Solve() {
	moves = LoadVec<int, int>(MAX);
	
	for (int i = 0; i < MAX; i++) {
		rotate(moves.begin(), moves.begin() + 1, moves.end());
		if (moves[2].first == moves[4].first && moves[3].first == moves[5].first) {
			area = moves[0].second * moves[1].second - moves[3].second * moves[4].second;
			break;
		}
	}
