...
//사용자 정의 함수
Pii FindMostNeutralPair(const vector<int>& liquids, const int N);
...
int main() {
    ...
    liquids = LoadVector<int>(N);
    Sort(liquids);
    neutralPair = FindMostNeutralPair(liquids, N);

    cout << neutralPair.first << " " << neutralPair.second;

    return 0;
}

Pii FindMostNeutralPair(const vector<int>& liquids, const int N) {
    Pii neutralPair = { liquids[0], liquids[N - 1] };
    int minPH = abs(liquids[0] + liquids[N - 1]);
    int curPH;
    int ptrL = 0, ptrR = N - 1;

    while (ptrL < ptrR) {
        curPH = abs(liquids[ptrL] + liquids[ptrR]);
        if (curPH < minPH) {
            minPH = curPH;
            neutralPair = make_pair(liquids[ptrL], liquids[ptrR]);
        }
        ((liquids[ptrL] + liquids[ptrR]) > 0) ? ptrR-- : ptrL++;
    }
