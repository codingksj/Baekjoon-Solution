...
// ---------- 사용자 정의 함수 및 연산자 ----------
void DFS(const int depth, const int w);
...
int main() {
    ...
    weights = LoadVector<int>(N);
    dp = Matrix2D<bool>(N + 1, MAX_W + 1, false);
    ...
    while (Q--) {
        cin >> beed;
        cout << (beed > MAX_W || !dp.m2[N][beed] ? 'N' : 'Y') << "\n";
    }
...
void DFS(const int depth, const int w) {
    ...
    DFS(depth + 1, w + weights[depth]);
    DFS(depth + 1, w);
    DFS(depth + 1, abs(w - weights[depth]));
};