    ...
    for (d[0] = 1; d[0] <= 9; d[0]++) {
        for (d[1] = 1; d[1] <= 9; d[1]++) {
            for (d[2] = 1; d[2] <= 9; d[2]++) {
                for (d[3] = 1; d[3] <= 9; d[3]++) {
                    for (int i = 0; i < MAX; i++) {
                        n[i] = Calc(d[i % 4], d[(i + 1) % 4], d[(i + 2) % 4], d[(i + 3) % 4]);
                    }
                    clocks.insert(*min_element(n.begin(), n.end()));
                }
            }
        }
    }
...
void Solve() {
    ...
    num = *min_element(n.begin(), n.end());
};

void Output() {
    for (auto it = clocks.begin(); it != clocks.end(); it++, cnt++) {
        if (*it == num) {
            cout << cnt << "\n";
            break;
        }
