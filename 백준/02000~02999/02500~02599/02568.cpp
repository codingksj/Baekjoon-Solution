...
// ---------- 전역 변수 ----------
Vi wires, L(MAX + 1), R(MAX + 1);
Vi tail, lis, cut, dp;
int N, idx, val, cnt, cnt2;
...
void Solve() {
    ...
    for (int i = 0; i < N; i++) {
        if (tail.empty() || tail.back() < wires[i]) {
            tail.push_back(wires[i]);
            dp[i] = tail.size();
        }
        else {
            auto it = lower_bound(tail.begin(), tail.end(), wires[i]);
            *it = wires[i];
            dp[i] = it - tail.begin() + 1;
        }
    }
    lis = Vi(cnt = tail.size());
    cut = Vi(cnt2 = N - cnt);
    for (int i = N - 1; i >= 0; i--) {
        if (dp[i] == cnt) {
            lis[--cnt] = wires[i];
