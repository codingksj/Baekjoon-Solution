...
//사용자 지정 함수
Matrix2D_i FillPaper(const int R, const int C, const int K);
vector<int> FindPartitions(const Matrix2D_i& paper, Matrix2D_b& bfsCheck, const Pii& limit);
int BFS(const Matrix2D_i& paper, Matrix2D_b& bfsCheck, const Pii& start, const Pii& limit);
...
int main() {
    ...
    paper = FillPaper(R, C, K);
    bfsCheck = Init2DVector<bool>(R, C, false);
    limit = { R,C };
    partitions = FindPartitions(paper, bfsCheck, limit);
...
Matrix2D_i FillPaper(const int R, const int C, const int K) {
    Matrix2D_i paper = Init2DVector<int>(R, C, 0);
    int startR, startC, endR, endC;

    for (int i = 0; i < K; i++) {
        cin >> startC >> startR >> endC >> endR;
...
vector<int> FindPartitions(const Matrix2D_i& paper, Matrix2D_b& bfsCheck, const Pii& limit) {
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (!paper[r][c] && !bfsCheck[r][c]) {
                start = { r, c };
                area = BFS(paper, bfsCheck, start, limit);
                partitions.push_back(area);
            }
        }
    }
    return partitions;
...
int BFS(const Matrix2D_i& paper, Matrix2D_b& bfsCheck, const Pii& start, const Pii& limit) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!isValidCoord || bfsCheck[nr][nc] || paper[nr][nc]) {
                continue;
            }
            area++;
            bfsCheck[nr][nc] = true;
