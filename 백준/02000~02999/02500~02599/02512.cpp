...
//사용자 정의 함수
LL FindMaxBudget(const vector<LL>& V, const LL N, const LL totalBudget);
bool CanDistribute(const vector<LL>& V, const LL maxBudget, const LL totalBudget);
...
int main() {
    ...
    vector<LL> V;
    LL totalBudget, maxBudget;
    int N;

    cin >> N;
    V = LoadVector<LL>(N);
    cin >> totalBudget;
    maxBudget = FindMaxBudget(V, N, totalBudget);
...
LL FindMaxBudget(const vector<LL>& V, const LL N, const LL totalBudget) {
    ...
    while (left <= right) {
        mid = (left + right) / 2;
        CanDistribute(V, mid, totalBudget) ? (left = mid + 1, maxBudget = mid) : (right = mid - 1);
    }
...
bool CanDistribute(const vector<LL>& V, const LL maxBudget, const LL totalBudget) {
    ...
    for (const LL& e : V) {
        sum += ((e < maxBudget) ? e : maxBudget);
    }

    canDistribute = sum <= totalBudget;
