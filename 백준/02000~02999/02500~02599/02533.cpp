...
int main() {
    ...
    visChk = InitVector<bool>(V + 1, false);
    tree = Matrix2D<int>(V + 1, 0, 0);
    dp = Matrix2D<int>(V + 1, 2, 0);
    ...
    cout << min(dp.m2[root][NORMAL], dp.m2[root][EARLY]) << "\n";
...
void DFS(const int v) {
    ...
    for (const int& child : tree.m2[v]) {
        if (!visChk[child]) {
            DFS(child);

            dp.m2[v][NORMAL] += dp.m2[child][EARLY];
            dp.m2[v][EARLY] += min(dp.m2[child][EARLY], dp.m2[child][NORMAL]);
        }
    }
};

void InsertEdge(const int v1, const int v2) {
    tree.m2[v1].push_back(v2);
};