...
bool CheckVaild(const int r, const int c, const int n);
...
void DFS(const int depth) {
    ...
    if (depth == zeroCount) {
        isFound = true;
        Print2DVector(sudoku.m2, "\n", "");
        return;
    }
    for (int n = 1; n <= 9; n++) {
        auto& [r, c] = zeros[depth];
        if (CheckVaild(r, c, n)) {
            sudoku.m2[r][c] = n;
            DFS(depth + 1);
            sudoku.m2[r][c] = 0;
...
bool CheckVaild(const int r, const int c, const int n) {
    ...
    for (int i = 0; i < 9; i++) {
        sr = 3 * (r / 3) + i / 3;
        sc = 3 * (c / 3) + i % 3;
        if (sudoku.m2[r][i] == n || sudoku.m2[i][c] == n || sudoku.m2[sr][sc] == n) {
            return false;
        }
    }
    return true;
