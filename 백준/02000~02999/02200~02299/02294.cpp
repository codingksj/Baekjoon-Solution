...
// ---------- 사용자 정의 함수 ----------
int FindMinCoin(const vector<int>& coins, const int N, const int K);
...
int main() {
    ...
    coins = LoadVector<int>(N);
    minCoin = FindMinCoin(coins, N, K);

    cout << (minCoin == MILLION ? -1 : minCoin) << "\n";

    return 0;
}

int FindMinCoin(const vector<int>& coins, const int N, const int K) {
    vector<int> dp = InitVector<int>(MAX + 1, MILLION);
