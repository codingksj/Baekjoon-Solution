...
//사용자 정의 함수
vector<LL> MakeDiffVector(const vector<LL>& V, const int N);
LL GCD(const LL a, const LL b);
LL FindMaxM(const vector<LL>& diffV);
vector<LL> FindFactors(const LL M);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<LL> V, diffV, possibleMs;
    LL maxM;
    int N;
    ...
    V = LoadVector<LL>(N);
    diffV = MakeDiffVector(V, N);
    maxM = FindMaxM(diffV);
    possibleMs = FindFactors(maxM);
    Sort(possibleMs);

    PrintVector(possibleMs, " ");
