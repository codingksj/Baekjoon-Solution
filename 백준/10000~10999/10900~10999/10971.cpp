...
// ---------- 사용자 정의 함수 및 연산자 ----------
int TSP(const int v, const int route);
...
int main() {
    ...
    w = Matrix2D<int>(N, N, 0);
    dp = Matrix2D<int>(N, 1 << N, -1);

    w.m2 = Load2DVector<int>(N, N);
...
int TSP(const int v, const int route) {
    if (route == (1 << N) - 1) {
        return w.m2[v][0] ? w.m2[v][0] : INF;
    }
    if (dp.m2[v][route] != -1) {
        return dp.m2[v][route];
    }

    dp.m2[v][route] = INF;

    for (int k = 0; k < N; k++) {
        if (!w.m2[v][k] || route & (1<<k)) {
            continue;
        }
        dp.m2[v][route] = min(dp.m2[v][route], TSP(k, route | 1 << k) + w.m2[v][k]);
    }
