...
// ---------- 사용자 정의 함수 및 연산자 ----------
Matrix2D<bool> LoadPalindromeInfo(const vector<int>& seq, const int N);
void DoQuery(const Matrix2D<bool>& dp, const int query);
...
int main() {
    ...
    seq = LoadVector<int>(N);
    dp = LoadPalindromeInfo(seq, N);
...
Matrix2D<bool> LoadPalindromeInfo(const vector<int>& seq, const int N) {
    Matrix2D<bool> dp(N + 1, N + 1, 0);

    for (int i = 1; i <= N; i++) {
        dp.m2[i][i] = true;
        dp.m2[i - 1][i] = (i > 1 && seq[i - 1] == seq[i - 2]);
    }

    for (int gap = 2; gap < N; gap++) {
        for (int i = 1; i + gap <= N; i++) {
            dp.m2[i][i + gap] = (seq[i - 1] == seq[i - 1 + gap]) && dp.m2[i + 1][i + gap - 1];
        }
    }
