...
// ---------- 사용자 정의 함수 ----------
int CalcMinDistance(const vector<Pii>& checkPoints, const int N);
int CalcManhattanDistance(const Pii& p1, const Pii& p2);
...
int main() {
    ...
    checkPoints = LoadVector<int, int>(N);
    minDistance = CalcMinDistance(checkPoints, N);
...
int CalcMinDistance(const vector<Pii>& checkPoints, const int N) {
    ...
    for (int i = 1; i < N - 1; i++) {
        ...
        curDistance = CalcManhattanDistance(cur, prev) + CalcManhattanDistance(next, cur);
        newDistance = CalcManhattanDistance(next, prev);
        curBenefit = curDistance - newDistance;
        maxBenefit = max(maxBenefit, curBenefit);
    }

    for (int i = 1; i < N; i++) {
        totalDistance += CalcManhattanDistance(checkPoints[i], checkPoints[i - 1]);
    }

    totalDistance -= maxBenefit;
