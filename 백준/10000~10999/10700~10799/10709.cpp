...
// ---------- 사용자 정의 함수 ----------
Matrix2D<int> MakeCloudInfo(const Words& clouds, const int R, const int C);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    Matrix2D<int> cloudsInfo;
    Words clouds;
    int R, C;
    ...
    clouds = LoadVector<string>(R);
    cloudsInfo = MakeCloudInfo(clouds, R, C);

    Print2DVector<int>(cloudsInfo.m2, "\n", " ");

    return 0;
}

Matrix2D<int> MakeCloudInfo(const Words& clouds, const int R, const int C) {
    Matrix2D<int> cloudsInfo = Matrix2D<int>(R, C, -1);
    int minute;
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            minute = 0;
            if (clouds[r][c] == 'c') {
                cloudsInfo.m2[r][c] = minute++;
                while (c < C - 1 && clouds[r][c + 1] != 'c') {
                    cloudsInfo.m2[r][++c] = minute++;
                }
            }
        }
