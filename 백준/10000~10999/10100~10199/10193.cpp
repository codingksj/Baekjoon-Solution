...
bool Input();
void Solve();
int Cost2Swap(const int sz);
void Output();

// ---------- 전역 변수 ----------
string word1, word2;
int TC, totalCost, cost;
...
void Solve() {
	totalCost = Cost2Swap(word1.size());
	return;
};
...
void Output() {
	...
	if (totalCost > 0) {
		cout << " earned " << totalCost << " coins.\n";
	}
	else if (totalCost < 0) {
		cout << " cost " << -totalCost << " coins.\n";
	}
	else {
		cout << " was FREE.\n";
	}
