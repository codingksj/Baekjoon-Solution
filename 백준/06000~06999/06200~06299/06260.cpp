...
// ---------- 전역 변수 ----------
Words alphas = { "ABC", "DEF", "GHI", "JKL", "MNO", "PQRS", "TUV", "WXYZ", "abc", "def", "ghi", "jkl", "mno", "pqrs", "tuv", "wxyz"};
string pwd, plainTxt;
int sz, pos, idx;
...
void Solve() {
    plainTxt = string(pwd.length(), '-');
    for (int i = 0; i < pwd.length(); i++) {
        for (const auto& alpha : alphas) {
            if ((pos = alpha.find(pwd[i])) != string::npos) {
                sz = alpha.length();
                idx = (pos - (i + 1) % sz + sz) % sz;
                plainTxt[i] = alpha[idx];
                break;
            }
        }
    }
