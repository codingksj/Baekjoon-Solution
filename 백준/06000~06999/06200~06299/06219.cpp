...
int CountPrimeWithD(const vector<int>& primes, const int start, const int end, const int digit);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<int> primes = LoadPrimes(4 * MILLION);
    int start, end, digit;
    int dCount;

    cin >> start >> end >> digit;

    dCount = CountPrimeWithD(primes, start, end, digit);
...
int CountPrimeWithD(const vector<int>& primes, const int start, const int end, const int digit) {
    int dCount = 0;
    int iStart = LowerBoundIndex(primes, start);
    int iEnd = UpperBoundIndex(primes, end);
    int prime;
