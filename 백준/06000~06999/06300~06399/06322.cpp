...
// ---------- 사용자 정의 함수 및 연산자 ----------
void PrintRightTriangle(const int TC, const int a, const int b, const int c);
...
void PrintRightTriangle(const int TC, const int a, const int b, const int c) {
    ...
    if (b == -1) {
        side = c * c > a * a ? sqrt(c * c - a * a) : -1;
        (side == -1) ?
            cout << "Impossible." << "\n" :
            cout << "b = " << fixed << setprecision(3) << side << "\n";
    }
    else if (a == -1) {
        side = c * c > b * b ? sqrt(c * c - b * b) : -1;
        (side == -1) ?
            cout << "Impossible." << "\n" :
            cout << "a = " << fixed << setprecision(3) << side << "\n";
    }
    else {
        side = sqrt(a * a + b * b);
        cout << "c = " << fixed << setprecision(3) << side << "\n";
    }
