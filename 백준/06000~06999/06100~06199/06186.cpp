...
bool Input() {
    ...
    grass = Words(R, string(C, 0));
    visChk = VVb(R, Vb(C, false));
...
void BFS(int startR, int startC) {
    grassQ.push(Pii(startR, startC));

    while (!grassQ.empty()) {
        auto [r, c] = grassQ.front();
        grassQ.pop();
        ...
        for (auto& [dr, dc] : DT4) {
            ...
            if (grass[nr][nc] != GRASS) {
                continue;
            }
            visChk[nr][nc] = true;
            grassQ.push(Pii(nr, nc));
        }
    }
};
