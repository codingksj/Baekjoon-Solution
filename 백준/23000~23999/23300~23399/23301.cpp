...
bool Input();
void Solve();
int CalcGood(const int S, const int E);
void Output();

// ---------- 전역 변수 ----------
VVPii times;
Pii best = Pii(0, 0);
int mxGood, curGood, mnS = 1001, mxE;
int N, T, K;
...
void Solve() {
    for (int s = mnS; s <= mxE - T; s++) {
        curGood = CalcGood(s, s + T);
        if (mxGood < curGood) {
            mxGood = curGood;
            best = Pii(s, s + T);
        }
...
int CalcGood(const int S, const int E) {
    int good = 0;
    for (const auto& st : times) {
        for (const auto& [s, e] : st) {
            if (e < S) {
                continue;
            }
            if (s > E) {
