...
// ---------- 전역 변수 ----------
Words danceParty, nxtDanceParty;
string cur, nxt, ready = ".omln", jump = "owln.", sit = "..oLn";
int mxLength;
...
bool Input() {
    danceParty = LoadVector<string>(5);

    return true;
};

void Solve() {
    mxLength = danceParty[0].length();
    nxtDanceParty = InitVector<string>(5, string(mxLength, '.'));
    ...
    for (int i = 0; i < mxLength; i++) {
        ...
        nxt = (cur == sit ? sit : (cur == jump ? ready : jump));
        for (int j = 0; j < 5; j++) {
            nxtDanceParty[j][i] = nxt[j];
        }
    }
    return;
}
