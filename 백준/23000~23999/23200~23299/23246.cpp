...
//사용자 지정 함수
vector<Player> LoadPlayers(const int N);
void PrintNumber(const vector<Player>& players);
...
int main() {
    ...
    players = LoadPlayers(N);

    Sort(players, greater<Player>());
    PrintNumber(players);
...
vector<Player> LoadPlayers(const int N) {
    ...
    int lead, speed, boulder;

    for (Player& player : players) {
        cin >> player.index >> lead >> speed >> boulder;

        player.product = lead * speed * boulder;
        player.sum = lead + speed + boulder;
    }
