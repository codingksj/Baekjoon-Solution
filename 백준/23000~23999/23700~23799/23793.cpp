...
bool Input();
void Solve();
int Dijkstra(const int srcV, const int destV, const int skipV);
void Output();

// ---------- 전역 변수 ----------
VVPii graph;
LL distXY, distYZ, distXZ;
int V, E;
int u, v, w;
int x, y, z;
...
bool Input() {
    ...
    graph = Init2DVector<Pii>(V + 1, 0, Pii());
...
void Solve() {
    distXY = Dijkstra(x, y, -1);
    distYZ = Dijkstra(y, z, -1);
    distXZ = Dijkstra(x, z, y);
};
...
void Output() {
    cout << ((distXY == INF || distYZ == INF) ? -1 : distXY + distYZ) << " ";
    cout << (distXZ == INF ? -1 : distXZ);
};
