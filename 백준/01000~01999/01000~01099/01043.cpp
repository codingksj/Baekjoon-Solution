...
// ---------- 전역 변수 ----------
Matrix2D<int> parties;
vector<int> knows, parents, party;
int V, group, N;
...
int main() {
    ...
    knows = LoadVector<int>(N);
    parents = InitArithmeticSeq<int>(V + 1, 0);

    for (const int& v : knows) {
        parents[v] = 0;
    }
...
void LoadParty() {
    ...
    party = LoadVector<int>(peopleNum);

    for (int i = 1; i < peopleNum; i++) {
        UnionRoot(party[0], party[i]);
    }

    parties.m2.push_back(party);
};
...
void CheckParty() {
    ...
    for (const vector<int>& p : parties.m2) {
        canLie = true;
        for (const int& v : p) {
            if (!canLie) {
                break;
