...
// ---------- 사용자 정의 함수 ----------
int FindMaxVisibleBuildings(const vector<LL>& V, const int N);
...
int main() {
    ...
    vector<LL> buildings;
    int N, maxVisibleCount;

    cin >> N;

    buildings = LoadVector<LL>(N);
    maxVisibleCount = FindMaxVisibleBuildings(buildings, N);
...
int FindMaxVisibleBuildings(const vector<LL>& V, const int N) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        minGradient = BILLION;
        for (int j = i - 1; j >= 0; j--) {
            curGradient = (double)(V[i] - V[j]) / (i - j);
            if (curGradient < minGradient) {
                curCount++;
                minGradient = curGradient;
            }
        }
        maxGradient = -BILLION;
        for (int j = i + 1; j < N; j++) {
