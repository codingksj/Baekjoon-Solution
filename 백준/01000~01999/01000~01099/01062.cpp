...
bool Input();
void Solve();
void DFS(const int alphaSet, const int idx, const int depth);
void Output();
// ---------- 열거형 상수 ----------

// ---------- 전역 변수 ----------
Vi words;
const string basic = "antic";
string word;
int N, K, mxRead;
...
void DFS(const int alphaSet, const int idx, const int depth) {
    if (depth == K - 5) {
        ...
        for (int i = 0; i < N; i++) {
            know += ((words[i] & alphaSet) == words[i]);
        }
        mxRead = max(mxRead, know);
        return;
    }
    for (int i = idx; i < 26; i++) {
        if (!(alphaSet & (1 << i))) {
            DFS(alphaSet | (1 << i), i, depth + 1);
        }
    }
};
