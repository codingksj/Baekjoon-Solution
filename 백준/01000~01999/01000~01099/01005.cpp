...
void Input() {
    ...
    times = LoadVector<int>(N);
    graph = Init2DVector<int>(N, 0, 0);
    inDeg = InitVector<int>(N, 0);
    dp = InitVector<int>(N, 0);
...
void TopologySort() {
    ...
    while(!q.empty() && inDeg[W]) {
        src = q.front();
        q.pop();

        for (const int& next : graph[src]) {
            dp[next] = max(dp[next], dp[src] + times[src]);
            if (--inDeg[next] == 0) {
                q.push(next);
            }
        }
