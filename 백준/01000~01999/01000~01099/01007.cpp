...
bool Input();
void Solve();
void DFS(int depth, int addCnt, int subCnt, int totX, int totY);
void Output();
...
void DFS(int depth, int addCnt, int subCnt, int totX, int totY) {
	if (depth == 2 * N) {
		LL dist = (LL)totX * totX + (LL)totY * totY;
		mnDist = min(mnDist, dist);
		return;
	}
	if (addCnt < N) {
		DFS(depth + 1, addCnt + 1, subCnt, totX + V[depth].first, totY + V[depth].second);
	}
	if (subCnt < N) {
		DFS(depth + 1, addCnt, subCnt + 1, totX - V[depth].first, totY - V[depth].second);
	}
};
