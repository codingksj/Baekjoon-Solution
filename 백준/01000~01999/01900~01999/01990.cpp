...
// ---------- 사용자 정의 함수 및 연산자 ----------
vector<int> LoadPrimes(const int start, const int end);
void PrintPrimePelindrome(const vector<int>& primes);
bool CheckPelindrome(const int N);
...
int main() {
    ...
    primes = LoadPrimes(start, end);
    PrintPrimePelindrome(primes);
...
vector<int> LoadPrimes(const int start, const int end) {
    vector<int> primes;
    vector<bool> isPrime;
    int N = min((int)1E+7, end);

    isPrime = InitVector<bool>(N + 1, true);
