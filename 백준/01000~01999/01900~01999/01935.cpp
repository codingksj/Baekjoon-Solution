...
// ---------- 사용자 정의 함수 및 연산자 ----------
double CalcRPN(const vector<double>& numbers, const string& expression);
double Operate(const char op, const double& n1, const double& n2);
...
int main() {
    ...
    numbers = LoadVector<double>(N);
    result = CalcRPN(numbers, expression);
