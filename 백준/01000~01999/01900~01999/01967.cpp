...
// ---------- 사용자 정의 함수 및 연산자 ----------
void InsertEdge(const int v1, const int v2, const int w);
void DFS(const int v, const int length);

// ---------- 열거형 상수 ----------

// ---------- 전역 변수 ----------
Matrix2D<Pii> tree;
vector<bool> visCheck;
int N;
int farthestV = 0;
int maxLength = -1;
int curLength;
...
int main() {
    ...
    tree = Matrix2D<Pii>(N, 0, { 0,0 });
    visCheck = InitVector<bool>(N, false);

    for (int i = 0; i < N - 1; i++) {
        cin >> v1 >> v2 >> w;
        InsertEdge(v1 - 1, v2 - 1, w);
        InsertEdge(v2 - 1, v1 - 1, w);
    }
...
void DFS(const int v, const int length) {
    ...
    for (const auto& [adjV, w] : tree.m2[v]) {
        DFS(adjV, length + w);
    }
};