...
// ---------- 사용자 정의 함수 및 연산자 ----------
vector<bool> LoadPrimeChecks(const int N);
string FindMinEdit(const int p1, const int p2);

int main() {
    cin.tie(nullptr);
    ios::sync_with_stdio(false);

    int TC, prime1, prime2;

    cin >> TC;

    while (TC--) {
        cin >> prime1 >> prime2;
        cout << FindMinEdit(prime1, prime2) << "\n";
    }
...
vector<bool> LoadPrimeChecks(const int N) {
    vector<bool> isPrimes = InitVector<bool>(N + 1, true);
...
string FindMinEdit(const int p1, const int p2) {
    vector<bool> isPrimes = LoadPrimeChecks(10 * THOUSAND);
    vector<bool> bfsCheck = InitVector<bool>(10 * THOUSAND, false);
    queue<Pii> q;
    string s;
    int curPrime;
    int nextPrime;
    ...
    while (!q.empty()) {
        ...
        for (int pos = 0; pos < 4; pos++) {
            for (int digit = 0; digit <= 9; digit++) {
                s = to_string(curPrime);
                s[pos] = (digit + '0');
                nextPrime = stoi(s);
                if (nextPrime < 1000 || bfsCheck[nextPrime] || !isPrimes[nextPrime]) {
                    continue;
                }
                bfsCheck[nextPrime] = true;
                q.push({ nextPrime, curCnt + 1 });
            }
        }
    }

    return (isFind ? to_string(curCnt) : "Impossible");
};