    ...
    for (int via = 0; via < V; via++) {
        for (int src = 0; src < V; src++) {
            for (int dest = 0; dest < V; dest++) {
                if (graph.m2[src][via] + graph.m2[via][dest] < graph.m2[src][dest]) {
                    graph.m2[src][dest] = graph.m2[src][via] + graph.m2[via][dest];
                }
            }
        }
    }
...
void FindMinCycle() {
    for (int v1 = 0; v1 < V; v1++) {
        for (int v2 = v1 + 1; v2 < V; v2++) {
            minCycle = min(minCycle, graph.m2[v1][v2] + graph.m2[v2][v1]);
        }
    }
};