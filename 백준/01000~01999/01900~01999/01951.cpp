...
//사용자 지정 함수
LL CalcStamp(const LL N);
LL Power(const LL base, const LL exp);
...
LL CalcStamp(const LL N) {
    ...
    int lowerLength = log10(N);
    int upperLength = log10(N) + 1;

    for (LL i = 1; i <= lowerLength; i++) {
        stamp += (i * 9 * Power(10, i - 1)) % MOD;
    }
    left = ((N - Power(10, lowerLength)) + 1) * upperLength;
    stamp = (stamp + left) % MOD;
