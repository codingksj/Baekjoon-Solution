...
//사용자 지정 함수
Pii FillSnailAndFindCoord(const int N, const int pos);
...
int main() {
    ...
    coord = FillSnailAndFindCoord(N, pos);
...
Pii FillSnailAndFindCoord(const int N, const int pos) {
    Matrix2D_i snailArr = Init2DVector<int>(N, N, 0);
    Pii coord = { 0,0 };
    ...
    while (count > 0) {
        ...
        for (int i = dir; i < dir + 4; i++) {
            nr = r + dr[i % 4];
            nc = c + dc[i % 4];
            if (nr >= 0 && nr < N && nc >= 0 && nc < N && !snailArr[nr][nc]) {
                r = nr;
                c = nc;
                dir = i % 4;
                break;
