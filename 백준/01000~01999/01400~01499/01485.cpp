...
//사용자 정의 함수
vector<int> MakeSides(const vector<Pii>& points);
int CalcDistance(const Pii& a, const Pii& b);
bool CheckSquarePossible(const vector<int>& sides);
...
int main() {
    ...
    while (TC--) {
        points = LoadPairVector<int, int>(4);
        Sort(points);
        sides = MakeSides(points);
        canMake = CheckSquarePossible(sides);
        cout << canMake << "\n";
    }
