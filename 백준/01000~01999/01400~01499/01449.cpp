...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CountMinTape(const vector<int>& holes, const int tapeLength);
...
int main() {
    ...
    vector<int> holes;
    int holeCount, tapeLength;
    int tapeCount;

    cin >> holeCount >> tapeLength;

    holes = LoadVector<int>(holeCount);
    SortAll(holes);
    tapeCount = CountMinTape(holes, tapeLength);
...
int CountMinTape(const vector<int>& holes, const int tapeLength) {
    int tapeCount = 1;
    int prevHole = holes[0];

    for (const int& curHole : holes) {
        (curHole - prevHole) < tapeLength ? 0 : (tapeCount++, prevHole = curHole);
    }
