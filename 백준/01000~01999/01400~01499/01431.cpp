...
LL CalcSumOfDigit(const string& s);
...
int main() {
    ...
    serials = LoadVector<string>(N);
    Sort(serials, Compare);
    PrintVector(serials, "\n");
...
bool Compare(const string& s1, const string& s2) {
    LL length1 = s1.length(), length2 = s2.length();
    LL sum1 = CalcSumOfDigit(s1), sum2 = CalcSumOfDigit(s2);
