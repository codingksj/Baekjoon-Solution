...
// ---------- 사용자 정의 함수 및 연산자 ----------
LL CountWithHand(const LL finger, const LL limit);
...
int main() {
    ...
    cin >> finger >> limit;
    cout << CountWithHand(finger, limit) << "\n";

    return 0;
}

LL CountWithHand(const LL finger, const LL limit) {
    vector<LL> w1 = { 8,6,4,2,8 };
    vector<LL> w2 = { 8,2,4,6,8 };
    LL n1 = limit - limit / 2;
    LL n2 = limit / 2;
    int idx = finger - 1;

    return idx + n1 * w1[idx] + n2 * w2[idx];
};