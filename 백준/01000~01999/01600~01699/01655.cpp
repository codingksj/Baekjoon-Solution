...
// ---------- 사용자 정의 함수 및 연산자 ----------
void TellNumber(const vector<int>& V);
...
void TellNumber(const vector<int>& V) {
    ...
    for (const int& e : V) {
        maxPQ.size() > minPQ.size() ? minPQ.push(e) : maxPQ.push(e);
        if (!(maxPQ.empty() || minPQ.empty()) && (maxPQ.top() > minPQ.top())) {
            maxV = maxPQ.top(), maxPQ.pop();
            minV = minPQ.top(), minPQ.pop();
