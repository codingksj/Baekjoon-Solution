...
//사용자 정의 함수
void UpdateEdge(Matrix2D_i& friends, const int vSrc, const int vDest);
void FloydWarshall(Matrix2D_i& friends, const int V);
int FindMinKevinBacon(Matrix2D_i& friends, const int V);
...
int main() {
    ...
    friends = Init2DVector<int>(V + 1, V + 1, BILLION);
    ...
    for (int e = 0; e < E; e++) {
        cin >> v1 >> v2;
        UpdateEdge(friends, v1, v2);
    }

    FloydWarshall(friends, V);
    index = FindMinKevinBacon(friends, V);
...
void UpdateEdge(Matrix2D_i& friends, const int vSrc, const int vDest) {
    friends[vSrc][vDest] = 1;
    friends[vDest][vSrc] = 1;
};
    ...
    for (int pass = 1; pass <= V; pass++) {
        for (int src = 1; src <= V; src++) {
            for (int dest = 1; dest <= V; dest++) {
                friends[src][dest] = min(friends[src][dest], friends[src][pass] + friends[pass][dest]);
            }
        }
    }
};
...
int FindMinKevinBacon(Matrix2D_i& friends, const int V) {
    ...
    for (int src = 1; src <= V; src++) {
        curKevinBacon = 0;
        for (int dest = 1; dest <= V; dest++) {
            curKevinBacon += friends[src][dest];
        }
        if (curKevinBacon < minKevinBacon) {
            minKevinBacon = curKevinBacon;
            index = src;
        }
    }
