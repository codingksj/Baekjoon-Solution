...
bool Input() {
    ...
    field = LoadVector<string>(M);
    bfsChk = VVb(M, Vb(N, false));
...
void BFS(const int sr, const int sc) {
    ...
    while (!q.empty()) {
        ...
        for (const auto& [dr, dc] : { Pii(-1,0), Pii(0,1),Pii(1,0), Pii(0,-1) }) {
            int nr = r + dr;
            int nc = c + dc;
            if (nr < 0 || nr >= M || nc < 0 || nc >= N) {
                continue;
            }
            if (bfsChk[nr][nc] || (field[nr][nc] != field[sr][sc])) {
                continue;
            }
            bfsChk[nr][nc] = true;
            x++;
