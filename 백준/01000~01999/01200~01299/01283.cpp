...
void Solve() {
    ...
    for (const auto& pos : positions) {
        int idx = option[pos] >= 'a' ? option[pos] - 'a' : option[pos] - 'A';
        if (!isUsed[idx] && !isDetermined) {
			isUsed[idx] = true;
            isDetermined = true;
            res = option.substr(0, pos) + "[" + option.substr(pos, 1) + "]" + option.substr(pos + 1);
			break;
        }
    }
    if (!isDetermined) {
        ...
        for(int j = 0; j < option.size(); j++) {
            ...
            int idx = option[j] >= 'a' ? option[j] - 'a' : option[j] - 'A';
            if (!isUsed[idx]) {
                isUsed[idx] = true;
                isDetermined = true;
                res = option.substr(0, j) + "[" + option.substr(j, 1) + "]" + option.substr(j + 1);
                break;
            }
		}
    }
