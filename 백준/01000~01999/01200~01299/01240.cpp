...
bool Input();
void Solve();
int BFS(const int vSrc, const int vDest);
void Output();
...
bool Input() {
    ...
    tree = Init2DVector<Pii>(V + 1, 0, Pii(0, 0));

    for (int i = 0; i < E; i++) {
        cin >> v1 >> v2 >> dist;

        tree[v1].push_back(Pii(v2, dist));
        tree[v2].push_back(Pii(v1, dist));
    }
...
void Solve() {
    ...
    for (int i = 0; i < M; i++) {
        ...
        dists.push_back(BFS(vSrc, vDest));
    }
}
...
int BFS(const int vSrc, const int vDest) {
    ...
    while (!q.empty()) {
        ...
        for (const auto& [nV, nDist] : tree[v]) {
            if (!bfsChk[nV]) {
                bfsChk[nV] = true;
                q.push(Pii(nV, dist + nDist));
            }
        }
    }
    return -1;
