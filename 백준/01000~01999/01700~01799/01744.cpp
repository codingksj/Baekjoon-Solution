...
// ---------- 전역 변수 ----------
priority_queue<int, Vi, greater<int>> nPq;
...
void Solve() {
    while (pPq.size() > 1) {
        int x = pPq.top(); pPq.pop();
        int y = pPq.top(); pPq.pop();
        sum += x * y;
    }
    while (nPq.size() > 1) {
        int x = nPq.top(); nPq.pop();
        int y = nPq.top(); nPq.pop();
        sum += x * y;
    }
    if (!pPq.empty()) {
        sum += pPq.top();
