...
template<typename T, typename Compare = less<T>> void Sort2D(vector<vector<T>>& V, Compare cmp = Compare()) { 
    for (vector<T>& row : V) { Sort(row, cmp); } 
}
...
void Dijkstra(const Matrix2D<Pii>& graph, vector<int>& dists, const int start);
void PrintShortestPathCosts(const vector<int>& dists);
...
int main() {
    ...
    graph = Matrix2D<Pii>(V, 0, { 0, INF });

    dists = InitVector<int>(V, INF);

    for (int i = 0; i < E; i++) {
        cin >> v1 >> v2 >> w;
        InsertEdge(graph, v1 - 1, v2 - 1, w);
    }
    Dijkstra(graph, dists, vStart - 1);
    PrintShortestPathCosts(dists);
