...
//사용자 지정 함수
Matrix2D_i MakeGraph(const int V, const int E);
void BFS(const Matrix2D_i& graph, vector<int>& bfsCheck, const int V);
bool CheckBipartite(const Matrix2D_i& graph, vector<int>& bfsCheck, const int V);
...
int main() {
    ...
    while (TC--) {
        ...
        graph = MakeGraph(V, E);
        bfsCheck = InitVector(V + 1, 0);
        BFS(graph, bfsCheck, V);
        isBipartite = CheckBipartite(graph, bfsCheck, V);
...
Matrix2D_i MakeGraph(const int V, const int E) {
    Matrix2D_i graph = Init2DVector(V + 1, 0, 0);
    int v, u;
...
void BFS(const Matrix2D_i& graph, vector<int>& bfsCheck, const int V) {
    ...
    for (int i = 1; i <= V; i++) {
        ...
        while (!q.empty()) {
            ...
            (bfsCheck[now] == RED) && (color = BLUE);
            (bfsCheck[now] == BLUE) && (color = RED);
