...
bool Input();
void Solve();
void DFS(const int MAX_DEPTH, int depth, int bishop, const bool IS_EVEN);
void Output();

// ---------- 전역 변수 ----------
VVi board;
VPii oddBishop, evenBishop;
Vb diagChk1, diagChk2;
int N, mxBishop, mxOdd, mxEven;
...
void Solve() {
    ...
    for (int r = 0; r < N; r++) {
        for (int c = 0; c < N; c++) {
            cin >> board[r][c];
            if (board[r][c] == EMPTY) {
                (((r + c) & 1) ? oddBishop : evenBishop).push_back(Pii(r, c));
            }
        }
    }
    DFS(evenBishop.size(), 0, 0, true);
    DFS(oddBishop.size(), 0, 0, false);
    mxBishop = mxEven + mxOdd;
    return;
};

void DFS(const int MAX_DEPTH, int depth, int bishop, const bool IS_EVEN) {
    if (depth == MAX_DEPTH) {
        IS_EVEN ? mxEven = max(mxEven, bishop) : mxOdd = max(mxOdd, bishop);
        return;
    }
    const auto& [r, c] = (IS_EVEN ? evenBishop : oddBishop)[depth];
    const int idx1 = r - c + (N - 1);
    const int idx2 = r + c;
    if (!(diagChk1[idx1] || diagChk2[idx2])) {
        diagChk1[idx1] = diagChk2[idx2] = true;
        DFS(MAX_DEPTH, depth + 1, bishop + 1, IS_EVEN); 
        diagChk1[idx1] = diagChk2[idx2] = false;
    }
    DFS(MAX_DEPTH, depth + 1, bishop, IS_EVEN);
};
