...
// ---------- 사용자 정의 함수 ----------
int CountRightTriangle(const vector<PLL>& points, const int N);
bool CheckRightTriangle(const PLL& p1, const PLL& p2, const PLL& p3);
LL CalcDistance(const PLL& p1, const PLL& p2);
...
int main() {
    ...
    points = LoadVector<LL, LL>(N);
    count = CountRightTriangle(points, N);
...
int CountRightTriangle(const vector<PLL>& points, const int N) {
    ...
    for (int i = 0; i < N - 2; i++) {
        for (int j = i + 1; j < N - 1; j++) {
            for (int k = j + 1; k < N; k++) {
                CheckRightTriangle(points[i], points[j], points[k]) && count++;
            }
        }
    }
...
bool CheckRightTriangle(const PLL& p1, const PLL& p2, const PLL& p3) {
    LL ab = CalcDistance(p1, p2);
    LL bc = CalcDistance(p2, p3);
    LL ca = CalcDistance(p3, p1);
