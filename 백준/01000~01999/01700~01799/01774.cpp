...
void Solve() {
    ...
    for (int i = 0; i < N - 1; i++) {
        for (int j = i + 1; j < N; j++) {
            E.push_back(Tdii(V[i] - V[j], i + 1, j + 1));
        }
    }
    SortAll(E);
    for (const auto& [cost, v1, v2] : E) {
        Union(v1, v2) && (minDist += cost);
    }
};
...
double operator-(const Pii& p1, const Pii& p2) {
    LL dr = p1.first - p2.first;
    LL dc = p1.second - p2.second;
