...
int FindMinLength(const vector<int>& pfV, const int N, const int S);
...
int main() {
    ...
    minLength = FindMinLength(pfV, N, S);
...
int FindMinLength(const vector<int>& pfV, const int N, const int S) {
    ...
    while (left < N) {
        sum = pfV[right] - pfV[left];
        length = right - left;
        (sum < S) ? 
            (right < N ? right++ : left++) : 
            (left++, minLength = min(minLength, length));
    }

    minLength = (minLength == N + 1) ? 0 : minLength;
