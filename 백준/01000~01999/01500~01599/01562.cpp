    ...
    for (int n = 2; n <= N; n++) {
        for (int d = 0; d < 10; d++) {
            for (int b = 0; b < (1 << 10); b++) {
                if (d == 0) {
                    dp[n][d][b | (1 << d)] = (dp[n][d][b | (1 << d)] + dp[n - 1][1][b]) % BILLION;
                    continue;
                }
                if (d == 9) {
                    dp[n][d][b | (1 << d)] = (dp[n][d][b | (1 << d)] + dp[n - 1][8][b]) % BILLION;
                    continue;
                }
                dp[n][d][b | (1 << d)] = (dp[n][d][b | (1 << d)] + dp[n - 1][d + 1][b]) % BILLION;
                dp[n][d][b | (1 << d)] = (dp[n][d][b | (1 << d)] + dp[n - 1][d - 1][b]) % BILLION;
            }
        }
    }
    for (int d = 1; d < 10; d++) {
        res = (res + dp[N][d][(1 << 10) - 1]) % BILLION;
    }
    return;
}
