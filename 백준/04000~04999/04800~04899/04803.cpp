...
// ---------- 템플릿 함수 ----------
template<typename T> vector<T> InitVector(const size_t SIZE, const T INIT = T()) {
    return vector<T>(SIZE, INIT);
}
template<typename T> vector<vector<T>> Init2DVector(const size_t R, const size_t C, const T INIT = T()) {
    return vector<vector<T>>(R, InitVector(C, INIT));
}
template<typename T> vector<vector<vector<T>>> Init3DVector(const size_t H, const size_t R, const size_t C, const T INIT = T()) {
...
Vi par, used;
set<int> unable;
int TC, V, E, v1, v2, treeCnt, caseNum;
...
void Solve() {
    ...
    for (int e = 0; e < E; e++) {
        cin >> v1 >> v2;
        if (!Union(--v1, --v2)) {
            unable.insert(Find(v1));
        };
    }
    for (v1 = 0; v1 < V; v1++) {
        v2 = Find(v1);
        !(unable.count(v2) || used[v2]) && treeCnt++;
        used[v2] = true;
    }
    return;
}
