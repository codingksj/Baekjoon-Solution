...
Pdd operator-(const Pdd& p1, const Pdd& p2);
Pdd operator*(const double& d, const Pdd& p);
void Output();

// ---------- 전역 변수 ----------
Pdd A, B, C, D, E, F, G, H, O;
Pdd vecAH, vecAC;
double EF, DF, DE, sDEF, s, sinA, cosA, AH;
double AB, BC, AC;
...
bool Input() {
    A = LoadPair<double, double>();
    B = LoadPair<double, double>();
    C = LoadPair<double, double>();
    D = LoadPair<double, double>();
    E = LoadPair<double, double>();
    F = LoadPair<double, double>();

    return !((A == O) && (B == O) && (C == O) && (D == O) && (E == O) && (F == O));
};
 
void Solve() {
    EF = Dist(E, F), DF = Dist(D, F), DE = Dist(D, E);
    AB = Dist(A, B), BC = Dist(B, C), AC = Dist(A, C);
    cosA = (pow(AB, 2) + pow(AC, 2) - pow(BC, 2)) / (2 * AB * AC);
    sinA = sqrt(1 - pow(cosA, 2));
    s = (DE + EF + DF) / 2;
    sDEF = sqrt(s * (s - DF) * (s - EF) * (s - DE));
    AH = sDEF / (AB * sinA);
    vecAC = C - A;
    vecAH = (AH / AC) * vecAC;
    H = A + vecAH;
...
Pdd operator*(const double& d, const Pdd& p) {
    return Pdd(d * p.first, d * p.second);
};

void Output() {
    PrintPair<double, double>(G, " ", " ");
    PrintPair<double, double>(H);
};
