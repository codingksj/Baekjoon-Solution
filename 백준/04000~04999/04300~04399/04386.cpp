...
// ---------- 사용자 정의 함수 및 연산자 ----------
Matrix2D<Pdi> MakeGraph(const vector<Pdd>& stars, const int N);
double CalcDist(const Pdd& star1, const Pdd& star2);
double Prim(const Matrix2D<Pdi>& graph, const int N);
...
int main() {
    ...
    stars = LoadVector<double, double>(N);
    graph = MakeGraph(stars, N);
    minCost = Prim(graph, N);
...
Matrix2D<Pdi> MakeGraph(const vector<Pdd>& stars, const int N) {
    Matrix2D<Pdi> graph(N, N, { 0, 0 });
    double dist;

    for (int p1 = 0; p1 < N; p1++) {
        for (int p2 = p1 + 1; p2 < N; p2++) {
            dist = CalcDist(stars[p1], stars[p2]);
            graph.m2[p1][p2] = { dist, p2 };
            graph.m2[p2][p1] = { dist, p1 };
        }
    }
...
double CalcDist(const Pdd& star1, const Pdd& star2) {
    double dr = star1.first - star2.first;
    double dc = star1.second - star2.second;
...
double Prim(const Matrix2D<Pdi>& graph, const int N) {
    ...
    vector<bool> visCheck = InitVector<bool>(N, false);
    double totalCost = 0;
    double curCost;
    int curV;
    ...
    while (!pq.empty()) {
        ...
        visCheck[curV] = true;
        totalCost += curCost;
        for (const auto& [nCost, nV] : graph.m2[curV]) {
            if (!visCheck[nV]) {
                pq.push({ nCost, nV });
            }
        }
