...
//사용자 지정 함수
int CountIsland(const Matrix2D_i& island, Matrix2D_b& bfsCheck, const Pii& limit);
void BFS(const Matrix2D_i& island, Matrix2D_b& bfsCheck, const Pii& limit, const Pii& start);
...
int main() {
    ...
    while (cin >> W >> H && (W || H)) {
        limit = { H,W };
        island = Load2DVector<int>(H, W);
        bfsCheck = Init2DVector<bool>(H, W, false);
        islandCount = CountIsland(island, bfsCheck, limit);
...
int CountIsland(const Matrix2D_i& island, Matrix2D_b& bfsCheck, const Pii& limit) {
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (!bfsCheck[r][c] && island[r][c]) {
                start = { r,c };
                BFS(island, bfsCheck, limit, start);
                islandCount++;
            }
        }
    }
...
void BFS(const Matrix2D_i& island, Matrix2D_b& bfsCheck, const Pii& limit, const Pii& start) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            if (isValidCoord && !bfsCheck[nr][nc] && island[nr][nc]) {
                bfsCheck[nr][nc] = true;
                q.push({ nr,nc });
            }
        }
