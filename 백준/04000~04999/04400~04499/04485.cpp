...
void Solve() {
    ...
    pq.push({ -cave[0][0], Pii(0,0) });

    while (!pq.empty()) {
        int cost = -pq.top().first;
        const auto [r, c] = pq.top().second;
        ...
        for (const auto& [dr, dc] : dt4) {
            ...
            int nCost = cost + cave[nr][nc];
            if (dists[nr][nc] > nCost) {
                dists[nr][nc] = nCost;
                pq.push({ -dists[nr][nc] , Pii(nr,nc) });
            }
        }
    }
};
