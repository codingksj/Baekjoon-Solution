...
bool Input();
void Solve();
void DFS(int depth, long double r, long double c);
void Output();

// ---------- 전역 변수 ----------
LL a, b, k;
int TC, L;
long double p, q, w, leftR, rightR, downC, upC;
bool isExist;
...
void Solve() {
    isExist = false;
    if (L == 0) {
        isExist = !(p < a || p > a + pow(3, k) || q < b || q > b + pow(3, k));
        return;
    }
    DFS(1, a, b);
    return;
...
void DFS(int depth, long double r, long double c) {
    ...
    for (int dr = 0; dr < 3; dr++) {
        for (int dc = 0; dc < 3; dc++) {
            double leftR = r + dr * w;
            double rightR = r + (dr + 1) * w;
            double downC = c + dc * w;
            double upC = c + (dc + 1) * w;
            if (p < leftR || p > rightR || q < downC || q > upC || (dr == 1 && dc == 1)) {
                continue;
            }
            if (depth == L) {
                isExist = true;
