...
//사용자 정의 함수
int CalcMinKnightMove(Matrix2D_b& bfsCheck, const Pii& limit, const Pii& src, const Pii& dest);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    Matrix2D_b chessBoard;
    Pii src, dest, limit;
    int TC, N;
    int minKnightMove;
    ...
    while (TC--) {
        ...
        chessBoard = Init2DVector<bool>(N, N, false);
        limit = { N, N };
        minKnightMove = CalcMinKnightMove(chessBoard, limit, src, dest);
...
int CalcMinKnightMove(Matrix2D_b& bfsCheck, const Pii& limit, const Pii& src, const Pii& dest) {
    ...
    vector<Pii> knightMoves = { {2, 1},{1, 2},{-1, 2},{-2, 1} };
    Pii curPos, newPos;
    ...
    while (!q.empty()) {
        curKnightMove = q.front().second;
        curPos = q.front().first;
        q.pop();
        if (curPos == dest) {
            minKnightMove = curKnightMove;
            break;
        }
        for (const Pii& knightMove : knightMoves) {
            newPos = curPos + knightMove;
            nr = newPos.first;
            nc = newPos.second;
            isValidCoord = (nr >= 0 && nr < R) && (nc >= 0 && nc < C);
            if (isValidCoord && !bfsCheck[nr][nc]) {
                q.push({ newPos, curKnightMove + 1 });
                bfsCheck[nr][nc] = true;
            }
            newPos = curPos - knightMove;
            nr = newPos.first;
