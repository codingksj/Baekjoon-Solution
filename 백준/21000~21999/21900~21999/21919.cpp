...
//사용자 지정 함수
vector<bool> MakePrimeChecks(const int N);
LL CalcPrimeLCM(const vector<bool>& isPrime, const vector<LL>& V);
...
int main() {
    ...
    vector<LL> V;
    vector<bool> isPrime = MakePrimeChecks(MILLION);
    LL primeLCM;
    int N;
    ...
    V = LoadVector<LL>(N);
    primeLCM = CalcPrimeLCM(isPrime, V);
...
LL CalcPrimeLCM(const vector<bool>& isPrime, const vector<LL>& V) {
    ...
    for (const LL& num : V) {
        (isPrime[num] && (primeLCM % num)) && (primeLCM *= num);
    }

    primeLCM = (primeLCM == 1) ? -1 : primeLCM;
