...
//사용자 정의 함수
Pii FindMe(const Words& campus, const Pii& limit);
int BFS(Words& campus, Matrix2D_b& bfsCheck, const Pii& start, const Pii& limit);
...
int main() {
    ...
    limit = { N,M };
    campus = LoadVector<string>(N);
    bfsCheck = Init2DVector<bool>(N, M, false);
    me = FindMe(campus, limit);
    people = BFS(campus, bfsCheck, me, limit);
    
    cout << (people ? to_string(people) : "TT") << "\n";
...
int BFS(Words& campus, Matrix2D_b& bfsCheck, const Pii& start, const Pii& limit) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (!isValidCoord || bfsCheck[nr][nc] || campus[nr][nc] == 'X') {
                continue;
            }
            (campus[nr][nc] == 'P') && people++; 
            bfsCheck[nr][nc] = true;
            q.push({ nr, nc });
        }
    }
