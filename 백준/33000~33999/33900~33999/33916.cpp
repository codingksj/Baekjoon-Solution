...
bool Input();
void Solve();
bool CheckCoprime(int n1, int r1, int n2, int r2);
...
void Solve() {
	isCoprime = CheckCoprime(n1, r1, n2, r2);
};
...
bool CheckCoprime(int n1, int r1, int n2, int r2) {
	for (auto p : primes) {
		...
		int cnt1 = CountPrime(n1, p) - CountPrime(r1, p) - CountPrime(n1 - r1, p);
		int cnt2 = CountPrime(n2, p) - CountPrime(r2, p) - CountPrime(n2 - r2, p);
		if (cnt1 && cnt2) {
			return false;
		}
	}
