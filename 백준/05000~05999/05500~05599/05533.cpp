...
// ---------- 사용자 정의 함수 및 연산자 ----------
Vi CalcGameScore(const int game);
...
int main() {
    ...
    scores = CalcGameScore(N);

    PrintVector<int>(scores, "\n");

    return 0;
}

Vi CalcGameScore(const int game) {
    VVi counts = Init2DVector<int>(3, 101, 0);
    VVi players = Init2DVector<int>(game, 3, 0);
    Vi scores = InitVector<int>(game, 0);
    int card;

    for (int g = 0; g < game; g++) {
        for (int round = 0; round < 3; round++) {
            cin >> players[g][round];
            counts[round][players[g][round]]++;
        }
    }
    for (int round = 0; round < 3; round++) {
        for (int g = 0; g < game; g++) {
            card = players[g][round];
            scores[g] += (counts[round][card] > 1 ? 0 : card);
        }
    }
