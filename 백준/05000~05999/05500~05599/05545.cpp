...
template<typename T> int LowerBoundIndex(const vector<T>& V, const T target) { const int index = lower_bound(V.begin(), V.end(), target) - V.begin(); return index; }
template<typename T> int UpperBoundIndex(const vector<T>& V, const T target) { const int index = upper_bound(V.begin(), V.end(), target) - V.begin(); return index; }
...
//사용자 지정 함수
int FindBestPizzaCaloriePerWon(const vector<int>& topingCals, const int topingPrice, const int doughCal, const int doughPrice);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<int> topingCals;
    int N, doughPrice, topingPrice, doughCal;
    int bestCaloriePerWon;

    cin >> N;
    cin >> doughPrice >> topingPrice;
    cin >> doughCal;

    topingCals = LoadVector<int>(N);

    Sort(topingCals, greater<int>());

    bestCaloriePerWon = FindBestPizzaCaloriePerWon(topingCals, topingPrice, doughCal, doughPrice);

    cout << bestCaloriePerWon << "\n";
...
int FindBestPizzaCaloriePerWon(const vector<int>& topingCals, const int topingPrice, const int doughCal, const int doughPrice) {
    ...
    int bestCaloriePerWon = doughCal / doughPrice;
    int caloriePerWon;

    for (const int& topingCal : topingCals) {
        totalCal += topingCal;
        totalPrice += topingPrice;
        caloriePerWon = totalCal / totalPrice;
        bestCaloriePerWon = max(bestCaloriePerWon, caloriePerWon);
    }

    return bestCaloriePerWon;
};