...
// ---------- 사용자 정의 함수 ----------
int FindMaxCommonStrLength(const string& s1, const string& s2);
...
int main() {
    ...
    maxLength = FindMaxCommonStrLength(s1, s2);
...
int FindMaxCommonStrLength(const string& s1, const string& s2) {
    ...
    for (int i = 1; i <= len1; i++) {
        for (int j = 1; j <= len2; j++) {
            if (s1[i - 1] == s2[j - 1]) {
                dp.m2[i][j] = dp.m2[i - 1][j - 1] + 1;
                maxLength = max(maxLength, dp.m2[i][j]);
            }
        }
    }
