...
bool Input() {
    ...
    graph = VVPii(V + 1, VPii(0, Pii(0, 0)));

    for (int e = 0; e < E; e++) {
        cin >> v1 >> v2 >> cost;
        graph[v1].push_back(Pii(v2, cost));
        graph[v2].push_back(Pii(v1, cost));
    }
...
void Dijkstra(const int start) {
    ...
    while (!pq.empty()) {
        const auto [v, cost] = pq.top();
        pq.pop();
        if (dists[v] < cost) {
            continue;
        }
        for (const auto& [nV, nCost] : graph[v]) {
            if (dists[nV] > dists[v] + nCost) {
                dists[nV] = dists[v] + nCost;
                pq.push(Pii(nV, nCost));
            }
        }
    }
};
