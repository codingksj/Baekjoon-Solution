...
// ---------- 사용자 정의 함수 및 연산자 ----------
void FastIO();	void SetUp();
bool Input();
void Solve();	bool Union(int u, int v, int w);	int Find(int v);
void Output();

// ---------- 전역 변수 ----------
vector<stack<int>> childs;
Vi par, W;
int N, M, a, b, pa, pb, x, w;
char q;
...
void Solve() {
	childs = InitVec<stack<int>>(N + 1);
	W = Vi(N + 1);
	par = InitArithmeticSeq<int>(N + 1, 0);
	for (int i = 1; i <= N; i++) {
		childs[i].push(i);
	}
	...
	for (int i = 0; i < M; i++) {
		...
		if (q == '!') {
			...
			pa = Find(a);
			pb = Find(b);
			cout << (pa == pb ? to_string(W[b] - W[a]) : "UNKNOWN") << "\n";
		}
	}
	return;
};
...
bool Union(int u, int v, int w) {
	...
	if (childs[pu].size() < childs[pv].size()) {
		swap(pu, pv);
		swap(u, v);
		w = -w;
	}
	par[pv] = pu;
	W[pv] = W[u] - W[v] + w;
	while (!childs[pv].empty()) {
		int nv = childs[pv].top(); childs[pv].pop();
		childs[pu].push(nv);
		W[nv] += (nv != pv) * W[pv];
	}
	return true;
