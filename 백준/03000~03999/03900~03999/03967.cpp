...
Words shape;
VPii star;
VPii starts = { Pii(1,1), Pii(3,1), Pii(0,4), Pii(1,7), Pii(0,4), Pii(1,1) };
VPii dt3 = { Pii(0,2), Pii(1,-1), Pii(1,1) };
int maxDepth;
Vb dfsChk;
bool isFound;
...
bool Input() {
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            ...
            if (isupper(shape[r][c])) {
                dfsChk[shape[r][c] - 'A'] = true;
            }
            else {
                star.push_back(Pii(r, c));
                maxDepth++;
            }
        }
    }
...
bool Check(){
    for (int i = 0; i < 6; i++) {
        const auto& [sr, sc] = starts[i];
        const auto& [dr, dc] = dt3[i / 2];
        int sum = 0;
        for (int d = 0; d < 4; d++) {
            sum += (shape[sr + dr * d][sc + dc * d] - 'A' + 1);
        }
        if (sum != SUM) {
            return false;
        }
