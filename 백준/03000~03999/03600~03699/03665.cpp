...
// ---------- 전역 변수 ----------
int inDeg[MAX_VERTEX + 1];
int res[MAX_VERTEX + 1];
int T[MAX_VERTEX + 1];
int rankOfT[MAX_VERTEX + 1];
int TC, N, M, a, b, emptySize;
bool isAdj[MAX_VERTEX + 1][MAX_VERTEX + 1];
bool isPossible, isOnly;
...
void Solve() {
	emptySize = sizeof(int) * N;

	memset(inDeg, 0, emptySize);
	memset(rankOfT, 0, emptySize);

	for (int i = 0; i < N; i++) {
		--T[i];
		inDeg[T[i]] = rankOfT[T[i]] =  i;
		for (int j = 0; j < N; j++) {
			isAdj[i][j] = false;
		}
	}
	...
	for (int i = 0; i < M; i++) {
		cin >> a >> b;
		--a, --b;
		if (rankOfT[a] > rankOfT[b]) {
			swap(a, b);
		}
		inDeg[a]++;
		inDeg[b]--;
		swap(isAdj[a][b], isAdj[b][a]);
	}
	TopologySort();
	return;
};
