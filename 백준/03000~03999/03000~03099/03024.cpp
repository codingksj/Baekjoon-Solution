...
void Solve() {
	for (int r = 0; r < N; r++) {
		for (int c = 0; c < N - 2; c++) {
			if (B[r][c] != '.' && B[r][c] == B[r][c + 1] && B[r][c + 1] == B[r][c + 2]) {
				winner = B[r][c];
				return;
			}
		}
	...
	for (int c = 0; c < N; c++) {
		for (int r = 0; r < N - 2; r++) {
			if (B[r][c] != '.' && B[r][c] == B[r + 1][c] && B[r + 1][c] == B[r + 2][c]) {
				winner = B[r][c];
				return;
			}
		}
	...
	for (int r = 0; r < N - 2; r++) {
		for (int c = 0; c < N - 2; c++) {
			if (B[r][c] != '.' && B[r][c] == B[r + 1][c + 1] && B[r + 1][c + 1] == B[r + 2][c + 2]) {
				winner = B[r][c];
				return;
			}
		}
...
void Output() {
	cout << (winner == '.' ? "ongoing" : string(1, winner)) << '\n';
	return;
};
