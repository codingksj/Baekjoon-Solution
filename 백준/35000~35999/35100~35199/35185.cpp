...
void Solve() {
	...
	for (int i = 0; i < positions.size() && res == LOSE; i++) {
		res = Check(positions[i]);
	}
	return;
};
...
Pii Check(int pos) {
	...
	for (int i = 0; i < L - 1; i++) {
		bool isL = moves[i] == 'L';
		nPos = isL ? max(nPos - 1, 1) : min(nPos + 1, N);
		visChk[nPos] = true;
	}
	bool isL = moves[L - 1] == 'L';
	nPos = isL ? max(nPos - 1, 1) : min(nPos + 1, N);
	return visChk[nPos] ? LOSE : Pii(pos, nPos);
};
