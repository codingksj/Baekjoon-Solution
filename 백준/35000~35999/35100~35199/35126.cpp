...
bool Input();
void Solve();
bool CheckC(int sr, int sc);
bool CheckU(int sr, int sc);
void Output();
...
bool CheckC(int sr, int sc) {
	bool isC = true;
	bool isRevC = true;
	if (sr + K - 1 >= N || sc + K - 1 >= M) {
		return false;
	}
	for (int c = sc; c < sc + K; c++) {
		if (P[sr][c] == '.' || P[sr + K - 1][c] == '.') {
			return false;
		}
	}
	for (int r = sr + 1; r < sr + K - 1; r++) {
		if (P[r][sc] != '#') {
			isC = false;
		}
		if (P[r][sc + K - 1] != '#') {
...
bool CheckU(int sr, int sc) {
	...
	for (int r = sr; r < sr + K; r++) {
		if (P[r][sc] == '.' || P[r][sc + K - 1] == '.') {
			return false;
		}
	}
	for (int c = sc + 1; c < sc + K - 1; c++) {
		if (P[sr][c] != '#') {
			isU = false;
		}
		if (P[sr + K - 1][c] != '#') {
