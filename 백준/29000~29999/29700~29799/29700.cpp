...
//사용자 지정 함수
int CalcPossibleArrangement(const int R, const int C, const int people);
...
int main() {
    ...
    int R, C, people;
    int possibleArrangement;

    cin >> R >> C >> people;

    possibleArrangement = CalcPossibleArrangement(R, C, people);

    cout << possibleArrangement << "\n";

    return 0;
}

int CalcPossibleArrangement(const int R, const int C, const int people) {
    int possibleArrangement = 0;
    int arrangement = 0;
    char seat;
    ...
    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            cin >> seat;
            (seat == '1') ?
                possibleArrangement += max(0, arrangement - people + 1), arrangement = 0 :
                arrangement++;
        }
        possibleArrangement += max(0, arrangement - people + 1);
        arrangement = 0;
    }
