...
bool Input();
void Solve();
int Convert(string score);
void Output();

// ---------- 전역 변수 ----------
Words G, grades = {"F","D0","D+","C0","C+","B0","B+","A0","A+"};
Vi C, scores = { 0, 100, 150, 200, 250, 300, 350, 400, 450};
string tmp, res = "impossible";
double X, gp, gradeSum;
int N, totG, creditSum;
...
bool Input() {
    ...
    C = InitVector<int>(N, 0);
    G = InitVector<string>(N, "F");
...
void Solve() {
    tmp = to_string(X);
    X = (tmp[0] - '0') * 100 + (tmp[2] - '0') * 10 + (tmp[3] - '0');
    for (int i = 0; i < N; i++) {
        gradeSum += C[i] * Convert(G[i]);
        creditSum += C[i];
    }
    for (const auto& grade : grades) {
        gp = Convert(grade);
        totG = (gradeSum + C[N - 1] * gp);
        if (totG / creditSum > X) {
            res = grade;
            break;
        }
