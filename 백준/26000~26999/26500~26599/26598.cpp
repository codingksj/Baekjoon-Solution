...
bool Input();
void Solve();
bool BFS(const int sr, const int sc);
void Output();
...
Words paper;
VVb bfsChk;
vector<Pii> dt4 = { Pii(-1,0),Pii(0,1),Pii(1,0),Pii(0,-1) };
int N, M;
bool isPretty = true;
...
bool Input() {
    ...
    paper = LoadVector<string>(N);
    bfsChk = VVb(N, Vb(M, false));
...
bool BFS(const int sr, const int sc) {
    ...
    int piece = 1;
    int w, h;
    auto [minR, minC] = start;
    auto [maxR, maxC] = start;
    ...
    while (!q.empty()) {
        ...
        for (const auto& [dr, dc] : dt4) {
            ...
            if (bfsChk[nr][nc] || paper[sr][sc] != paper[nr][nc]) {
                continue;
            }
            bfsChk[nr][nc] = true;
            q.push(Pii(nr, nc));
