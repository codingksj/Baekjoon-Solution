...
// ---------- 사용자 정의 함수 및 연산자 ----------
bool CheckVaccine(Matrix2D<int>& tissue1, Matrix2D<int>& tissue2, const int R, const int C);
void SpreadVaccine(Matrix2D<int>& tissue, const Pii& start, const Pii& limit, const int prevData, const int newData);

int main() {
    cin.tie(nullptr);
    ios::sync_with_stdio(false);

    Matrix2D<int> tissue1, tissue2;
    int R, C;

    cin >> R >> C;

    tissue1.m2 = Load2DVector<int>(R, C);
    tissue2.m2 = Load2DVector<int>(R, C);

    cout << (CheckVaccine(tissue1, tissue2, R, C) ? "YES" : "NO") << "\n";
...
bool CheckVaccine(Matrix2D<int>& tissue1, Matrix2D<int>& tissue2, const int R, const int C) {
    Pii start, limit;
    bool canBeVaccine = true;
    bool isVaccinized = false;

    for (int r = 0; r < R; r++) {
        for (int c = 0; c < C; c++) {
            if (tissue1.m2[r][c] == tissue2.m2[r][c]) {
                continue;
            }
            if (isVaccinized) {
                r = R;
            ...
            start = { r,c };
            limit = { R,C };
            SpreadVaccine(tissue1, start, limit, tissue1.m2[r][c], tissue2.m2[r][c]);
            isVaccinized = true;
        }
    }
...
void SpreadVaccine(Matrix2D<int>& tissue, const Pii& start, const Pii& limit, const int prevData, const int newData) {
    Matrix2D<bool> bfsCheck;
    queue<Pii> q, updateCoords; 
    ...
    bfsCheck = Matrix2D<bool>(R, C, false);
    q.push(start);
    bfsCheck.m2[r][c] = true;
    
    while (!q.empty()) {
        r = q.front().first;
        c = q.front().second;
        q.pop();
        tissue.m2[r][c] = newData;

        for (int i = 0; i < 4; i++) {
            nr = r + dr[i];
            nc = c + dc[i];
            isValidCoord = nr >= 0 && nr < R && nc >= 0 && nc < C;
            if (isValidCoord && !bfsCheck.m2[nr][nc] && (tissue.m2[nr][nc] == prevData)) {
                q.push({ nr,nc });
                bfsCheck.m2[nr][nc] = true;
            }
        }
    }
};