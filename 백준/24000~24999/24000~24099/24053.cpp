...
void InsertionSort() {
    ...
    for (int i = 1; i < N; i++) {
        ...
        for (pos = i - 1, key = A[i]; pos >= 0 && A[pos] > key; pos--) {
            prev = (A[pos] == B[pos]) + (A[pos + 1] == B[pos + 1]);
            A[pos + 1] = A[pos];
            cur = (A[pos] == B[pos]) + (A[pos + 1] == B[pos + 1]);
            cnt += (cur - prev);
            if (cnt == N) {
                isSame = true;
                return;
        ...
        if (pos + 1 != i) {
            prev = (A[i] == B[i]) + (A[pos + 1] == B[pos + 1]);
            A[pos + 1] = key;
            cur = (A[i] == B[i]) + (A[pos + 1] == B[pos + 1]);
            cnt += (cur - prev);
            if (cnt == N) {
                isSame = true;
                return;
