...
// ---------- 전역 변수 ----------
Vi scores(3), goals, turnOvers(3, 1);
int N, drawCnt = 1, mxTurnOver = 1;
int prv, cur;
...
void Solve() { 
    ...
    for (int i = 1; i < N; i++) {
        scores[cur = goals[i]]++;
        (scores[1] == scores[2]) && drawCnt++;
        if (prv == cur) {
            turnOvers[cur]++;
        }
        else {
            if (scores[prv] > scores[cur]) {
                mxTurnOver = max(mxTurnOver, turnOvers[prv]);
            }
            turnOvers[prv] = turnOvers[cur] = 1;
            prv = cur;
        }
    }
    if (abs(turnOvers[1] - turnOvers[2])) {
        mxTurnOver = max(mxTurnOver, scores[1] > scores[2] ? turnOvers[1] : turnOvers[2]);
    }
    return;
}
