...
// ---------- 사용자 정의 함수 및 연산자 ----------
void InsertEdge(Matrix2D<Pii>& graph, const int srcV, const int destV, const int weight);
vector<int> Dijkstra(const Matrix2D<Pii>& graph, const int start, const int V);
vector<int> FindCityDistK(const vector<int>& dists, const int V, const int K);
...
int main() {
    ...
    Matrix2D<Pii> graph;
    vector<int> dists, cities;
    int V, E, K, start;
    int srcV, destV, weight = 1;

    cin >> V >> E >> K >> start;

    graph = Matrix2D<Pii>(V, 0, { 0,0 });

    for (int e = 0; e < E; e++) {
        cin >> srcV >> destV;
        InsertEdge(graph, srcV - 1, destV - 1, weight);
    }

    dists = Dijkstra(graph, start - 1, V);
    cities = FindCityDistK(dists, V, K);

    if (cities.empty()) {
        cout << -1 << "\n";
    }
    else {
        PrintVector<int>(cities, "\n");
    }
...
void InsertEdge(Matrix2D<Pii>& graph, const int srcV, const int destV, const int weight) {
    graph.m2[srcV].push_back({ destV, weight });
};
...
vector<int> Dijkstra(const Matrix2D<Pii>& graph, const int start, const int V) {
    ...
    while (!pq.empty()) {
        ...
        if (dists[curV] < curDist) {
            continue;
        }

        for (const Pii& next : graph.m2[curV]) {
            newDist = curDist + next.second;
            newV = next.first;
            if (newDist < dists[newV]) {
                dists[newV] = newDist;
                pq.push({ newDist, newV });
            }
        }
    }
