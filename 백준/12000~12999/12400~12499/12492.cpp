...
const VPii SQ_DT4 = { Pii(0,0), Pii(0,1), Pii(1,0), Pii(1,1) };
const int MAX = 100 * MILLION;
...
void Solve() {
	...
	for (int r = 0; r < R; r++) {
		for (int c = 0; c < C; c++) {
			...
			for (int i = 0; i < 4; i++) {
				int nr = r + SQ_DT4[i].first;
				int nc = c + SQ_DT4[i].second;
				if (nr >= R || nc >= C || pic[nr][nc] != '#') {
					canChange = false;
					return;
				}
				pic[nr][nc] = (i / 2) ^ (i & 1) ? '\\' : '/';
