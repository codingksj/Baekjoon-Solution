...
bool Input();
void Solve();
bool CanMake(int l, int r, bool isForward);
void Output();
...
bool CanMake(int l, int r, bool isForward) {
	...
	char head = isForward ? T[l] : T[r];
	char tail = isForward ? T[r] : T[l];
	if (head != 'B' && tail != 'A') {
		return false;
	}
	if (tail == 'A' && CanMake(isForward ? l : l + 1, isForward ? r - 1 : r, isForward)) {
		return true;
	}
	if (head == 'B' && CanMake(isForward ? l + 1 : l, isForward ? r : r - 1, !isForward)) {
		return true;
	}
	return false;
};
