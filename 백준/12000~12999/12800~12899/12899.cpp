...
bool Input();
void Solve();
void Update(int node, int st, int ed, int idx);
int Query(int node, int st, int ed, int k);
void Output();

// ---------- 전역 변수 ----------
Vi counts(MAX + 1), tree(4 * MAX);
int N, x, l, q;
...
int Query(int node, int st, int ed, int k) {
    ...
    int mid = (st + ed) / 2;
    int sumL = tree[2 * node];
    if (k <= sumL) {
        return Query(2 * node, st, mid, k);
    }
    else {
        return Query(2 * node + 1, mid + 1, ed, k - sumL);
    }
};
