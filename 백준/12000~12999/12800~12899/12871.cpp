...
bool CheckCanMakeSameStr(const string& s1, const string& s2);
...
int main() {
    ...
    cout << CheckCanMakeSameStr(s1, s2) << "\n";
...
bool CheckCanMakeSameStr(const string& s1, const string& s2) {
    ...
    int lenS1 = s1.length();
    int lenS2 = s2.length();
    int lenMax = LCM(lenS1, lenS2);

    for (int i = 0; i < lenMax; i += lenS1) {
        infS1 += s1;
    }
    for (int i = 0; i < lenMax; i += lenS2) {
        infS2 += s2;
    }
