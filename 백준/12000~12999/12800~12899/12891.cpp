...
bool Input() {
    ...
    counts = Vi(4, 0);
    min_limits = LoadVector<int>(4);

    return true;
};

void Solve() {
    dna = full_dna.substr(0, P);
    canMake = true;

    for (const char& rna : dna) {
        for (int i = 0; i < 4; i++) {
            (rna == acgt[i]) && (counts[i]++);
        }
    }
    ...
    for (int i = 0; i < 4; i++) {
        if (counts[i] < min_limits[i]) {
            canMake = false;
        }
    }
    passwordCnt += canMake;

    for (int pos = P; pos < S; pos++) {
        canMake = true;
        for (int i = 0; i < 4; i++) {
            (full_dna[pos] == acgt[i]) && (counts[i]++);
            (full_dna[pos - P] == acgt[i]) && (counts[i]--);
            if (counts[i] < min_limits[i]) {
                canMake = false;
            }
        }
