...
//사용자 지정 함수
LL FindMaxProfit(const vector<LL>& profits, const int N, const int M);
...
int main() {
    ...
    profits = LoadVector<LL>(N);
    maxProfit = FindMaxProfit(profits, N, M);
...
LL FindMaxProfit(const vector<LL>& profits, const int N, const int M) {
    ...
    for (int i = M; i < N; i++) {
        curProfit += (profits[i] - profits[i - M]);
