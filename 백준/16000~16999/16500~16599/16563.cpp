...
//사용자 정의 함수
vector<int> LoadMinFactors(const int N);
void Factorize(const vector<int>& minFactors, const int N);

int main() {
    cin.tie(NULL);
    ios::sync_with_stdio(false);

    vector<int> minFactors = LoadMinFactors(5 * MILLION);
    int TC, N;
...
vector<int> LoadMinFactors(const int N) {
    vector<int> minFactors = InitVector<int>(N + 1, 0);
