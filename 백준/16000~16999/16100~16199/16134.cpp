...
bool Input();
void Solve();
TLL3 ExtGCD(LL a, LL b);
void Output();

// ---------- 전역 변수 ----------
LL N, R, fN = 1, fR = 1, fNR = 1, res = 1;
...
void Solve() {
	...
	auto [_0, invFactNR, _1] = ExtGCD(fNR, MOD);
	auto [_2, invFactR, _3] = ExtGCD(fR, MOD);
	invFactNR = (invFactNR + MOD) % MOD;
	invFactR = (invFactR + MOD) % MOD;
	res = fN * invFactNR % MOD;
	res = res * invFactR % MOD;
	return;
};
