...
//사용자 지정 함수
Matrix2D_i MakeLetterCounts(const string& word);
void DoQuery(const Matrix2D_i& letterCounts, const int Q);
...
int main() {
    ...
    string word;
    Matrix2D_i letterCounts;
    int Q;

    cin >> word;

    letterCounts = MakeLetterCounts(word);

    cin >> Q;

    DoQuery(letterCounts, Q);
...
Matrix2D_i MakeLetterCounts(const string& word) {
    ...
    letterCounts.reserve(maxL);

    for (const char& letter : word) {
        index = letter - 'a';
        curCount[index]++;
        letterCounts.push_back(curCount);
    }
...
void DoQuery(const Matrix2D_i& letterCounts, const int Q) {
    ...
    for (int i = 0; i < Q; i++) {
        ...
        index = alpha - 'a';
        val1 = (left > 0) ? letterCounts[left - 1][index] : 0;
        val2 = letterCounts[right][index];
        count = val2 - val1;
