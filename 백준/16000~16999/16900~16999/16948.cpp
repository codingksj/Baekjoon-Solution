...
// ---------- 전역 변수 ----------
VVb bfsChk;
vector<Pii> dt6 = { Pii(-2,-1),Pii(-2,1), Pii(0,-2), Pii(0,2), Pii(2,-1), Pii(2,1) };
int N, R1, C1, R2, C2;
int minMove = -1;
...
bool Input() {
    ...
    bfsChk = Init2DVector<bool>(N, N, false);
...
void Solve() {
    ...
    while (!q.empty()) {
        const auto [r, c, mv] = q.front();
        q.pop();
        if (r == R2 && c == C2) {
            minMove = (minMove == -1 ? mv : min(minMove, mv));
            continue;
        }
        for (const auto& [dr, dc] : dt6) {
            int nr = r + dr;
            int nc = c + dc;
            if (nr < 0 || nc < 0 || nr >= N || nc >= N || bfsChk[nr][nc]) {
                continue;
            }
            q.push(Tiii(nr, nc, mv + 1));
            bfsChk[nr][nc] = true;
        }
    }
};
