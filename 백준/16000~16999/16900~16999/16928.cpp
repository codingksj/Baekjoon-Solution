...
//사용자 지정 함수
vector<int> MakeWarp(const int N);
int CalcMinRoll(const vector<int>& warp, vector<bool>& bfsCheck, const int start, const int limit);
...
int main() {
    ...
    warp = MakeWarp(N + M);
    bfsCheck = InitVector<bool>(101, false);
    start = 1;
    limit = 101;
    minRoll = CalcMinRoll(warp, bfsCheck, start, limit);
...
vector<int> MakeWarp(const int N) {
    vector<int> warp = InitVector<int>(101, 0);
    int src, dest;
...
int CalcMinRoll(const vector<int>& warp, vector<bool>& bfsCheck, const int start, const int limit) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 1; i <= 6; i++) {
            newPos = curPos + i;
            isValidCoord = newPos > 0 && newPos < limit;
            if (isValidCoord && !bfsCheck[newPos]) {
                while (warp[newPos]) {
                    newPos = warp[newPos];
                }
                isValidCoord = newPos > 0 && newPos < limit;
                if (isValidCoord && !bfsCheck[newPos]) {
                    q.push({ newPos, curRoll + 1 });
                    bfsCheck[newPos] = true;
                }
            }
        }
    }
