...
//사용자 정의 함수
int FindMostProfitablePattern(const int H, const int Y);
...
int main() {
    ...
    profit = FindMostProfitablePattern(seed, year);
...
int FindMostProfitablePattern(const int H, const int Y) {
    const double patternA = 1.05;
    const double patternB = 1.2;
    const double patternC = 1.35;
    vector<int> dp(16, 0);
    int profit;
    ...
    for (int y = 0; y < Y; y++) {
        dp[5 + y] = max({ dp[4 + y] * patternA, dp[2 + y] * patternB, dp[y] * patternC });
    }
