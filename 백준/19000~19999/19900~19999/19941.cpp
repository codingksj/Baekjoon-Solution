...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CountMaxBurgerEat(const string& table, const int tableLength, const int radius);
...
int main() {
    ...
    string table;
    int tableLength, radius;

    cin >> tableLength >> radius;
    cin.ignore();
    cin >> table;

    cout << CountMaxBurgerEat(table, tableLength, radius);
...
int CountMaxBurgerEat(const string& table, const int tableLength, const int radius) {
    ...
    for (int i = 0; i < tableLength; i++) {
        left = max(i - radius, 0);
        right = min(i + radius, tableLength - 1);
        if (tb[i] != 'P') {
            continue;
        }
