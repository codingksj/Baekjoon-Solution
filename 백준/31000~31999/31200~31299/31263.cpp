...
// ---------- 사용자 정의 함수 및 연산자 ----------
int CountMinSoldier(const string& paper, const int N);
int CalcGap(const string& paper, const int N, const int idx);
...
int CountMinSoldier(const string& paper, const int N) {
    ...
    for (int i = 0, gap = 0; i < N; i += gap, soldier++) {
        gap = CalcGap(paper, N, i);
    }
...
int CalcGap(const string& paper, const int N, const int idx) {
    for (int gap = 3; gap >= 1; gap--) {
        if ((idx + gap == N || (idx + gap < N && paper[idx + gap] != '0')) && Check(paper.substr(idx, gap))) {
            return gap;
        }
    }
};
