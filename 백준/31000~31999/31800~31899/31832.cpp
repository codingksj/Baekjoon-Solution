...
bool Input();
void Solve();
bool NameChecker(string teamName);
void Output();
...
bool NameChecker(string teamName) {
    int alphaCnt = 0, numCnt = 0, len = teamName.length();
    if (len > 10) {
        return false;
    }
    for (const auto& ch : teamName) {
        if (isalpha(ch)) {
            isupper(ch) ? alphaCnt-- : alphaCnt++;
        }
        if (isdigit(ch)) {
            numCnt++;
        }
    }
    return (alphaCnt >= 0) && (numCnt < len);
};
