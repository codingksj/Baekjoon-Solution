...
void SetUp();
bool Input();
int Change(const string& t);
void Solve();
void DFS(const int idx, const int depth, int ed);
void Output();
...
bool Input() {
    ...
    for (int g = 0; g < G; g++) {
        ...
        while (N--) {
            cin >> C >> D >> s >> e;
            V.push_back(Info(Change(s), Change(e), C, g));
        }
    }
    mxDepth = V.size();
...
int Change(const string& t) {
    ...
    return D * DAY * HOUR + hour * HOUR + minute;
};
...
void DFS(const int idx, const int depth, int ed) {
    if (depth == mxDepth || sum >= MAX_CREDIT) {
        cnt += (sum == MAX_CREDIT);
        return;
    }
    for (int i = idx; i < mxDepth; i++) {
        const auto& [s, e, c, g] = V[i];
        if (ed <= s && !used[g] && sum < MAX_CREDIT) {
            sum += c;
            used[g] = true;
            DFS(i, depth + 1, e);
            used[g] = false;
