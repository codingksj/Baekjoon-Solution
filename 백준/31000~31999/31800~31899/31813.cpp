...
const LL INF = 111111111111111111LL;
...
void Solve() {
    for (size_t i = 0; K > 0 && i < sumOf10.size(); i++) {
        LL d = K / sumOf10[i];
        if (!d) {
            continue;
        }
        if (d == 10) {
            digits.push_back(9 * sumOf10[i]);
            digits.push_back(sumOf10[i]);
        }
        else {
            digits.push_back(d * sumOf10[i]);
        }
        K -= d * sumOf10[i];
    }
};
