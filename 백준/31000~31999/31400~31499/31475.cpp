...
void Solve() {
    ...
    while (!TT[r][c]) {
        ...
        for (int i = dir; i < dir + 4; i++) {
            nr = r + dr[i % 4];
            nc = c + dc[i % 4];
            if (nr < 0 || nr >= N || nc < 0 || nc >= M || TT[nr][nc]) {
                continue;
            }
            r = nr;
            c = nc;
    ...
    if (isUD) {
        for (int r = 0; r < N; r++) {
            for (int c = 0; c < M / 2; c++) {
                TT[r][c] ? (TT[r][M - 1 - c] = TT[r][c]) : (TT[r][c] = TT[r][M - 1 - c]);
            }
        }
    }
    if (isLR) {
        for (int r = 0; r < N / 2; r++) {
            for (int c = 0; c < M; c++) {
                TT[r][c] ? (TT[N - 1 - r][c] = TT[r][c]) : (TT[r][c] = TT[N - 1 - r][c]);
            }
        }
    }
};
